...
@st.cache_data(ttl=300)
def get_nws_hourly_series(lat, lon, tz_name, target_date):
    """Latest NWS hourly temperature forecast, restricted to one local calendar date."""
    local_tz = ZoneInfo(tz_name)
    point = get_json(f"https://api.weather.gov/points/{lat},{lon}")
    hourly_url = point["properties"]["forecastHourly"]
    hourly = get_json(hourly_url)
    rows = []
    for period in hourly.get("properties", {}).get("periods", []):
        try:
            dt = datetime.fromisoformat(period["startTime"]).astimezone(local_tz)
        except Exception:
            continue
        ...
        if period.get("temperatureUnit") == "C":
            temp = temp * 9 / 5 + 32
        rows.append({"time": dt, "temperature_f": temp})
    return rows


@st.cache_data(ttl=180)
def get_nws_observation_series(station_id, tz_name, target_date):
    """NWS station observations for one local calendar date, through now when target_date is today."""
    if not station_id:
        return []
    tz = ZoneInfo(tz_name)
    start_local = datetime.combine(target_date, datetime.min.time(), tzinfo=tz)
    ...
    if target_date > now_local.date():
        return []
    end_local = now_local if target_date == now_local.date() else datetime.combine(target_date, datetime.max.time(), tzinfo=tz)
    params = {"start": start_local.isoformat(), "end": end_local.isoformat(), "limit": 500}
    data = get_json(f"https://api.weather.gov/stations/{station_id}/observations", params=params)
    rows = []
    for feature in data.get("features", []):
        props = feature.get("properties") or {}
        ...
        try:
            ...
            continue
        rows.append({"time": dt, "temperature_f": temp_f})
    rows.sort(key=lambda x: x["time"])
    return rows


def render_intraday_temperature_chart(lat, lon, station_id, tz_name, target_date):
    """Render observed temperatures so far and the latest NWS hourly prediction over local time."""
    tz = ZoneInfo(tz_name)
    now_local = datetime.now(tz)
    try:
        forecast_rows = get_nws_hourly_series(lat, lon, tz_name, target_date)
    except Exception:
        forecast_rows = []
    try:
        observation_rows = get_nws_observation_series(station_id, tz_name, target_date)
    except Exception:
        observation_rows = []

    parts = []
    if forecast_rows:
        f = pd.DataFrame(forecast_rows)
        f["series"] = "NWS forecast"
        parts.append(f)
    if observation_rows:
        o = pd.DataFrame(observation_rows)
        o["series"] = "Observed"
        parts.append(o)
    if not parts:
        st.caption("Hourly NWS forecast/observation data is unavailable right now.")
        return
    ...
    base = alt.Chart(chart_df).encode(
        x=alt.X("time:T", title="Local time", axis=alt.Axis(format="%-I %p", labelAngle=0, tickCount=6)),
        y=alt.Y("temperature_f:Q", title="Temperature (°F)", scale=alt.Scale(zero=False)),
        color=alt.Color(
            "series:N",
            title=None,
            scale=alt.Scale(domain=["NWS forecast", "Observed"], range=["#c998ff", "#ff9bc8"]),
        ),
        tooltip=[
            alt.Tooltip("series:N", title="Series"),
            alt.Tooltip("time:T", title="Local time", format="%-I:%M %p"),
            alt.Tooltip("temperature_f:Q", title="Temperature", format=".1f"),
        ],
    )
    lines = base.mark_line(point=True, strokeWidth=2.4)

    layers = [lines]
    if target_date == now_local.date():
        now_df = pd.DataFrame({"time": [now_local]})
        now_rule = alt.Chart(now_df).mark_rule(strokeDash=[5, 5], opacity=.75).encode(x="time:T")
        now_text = alt.Chart(now_df).mark_text(align="left", dx=5, dy=-120, fontSize=11).encode(
            x="time:T", text=alt.value("NOW")
        )
        layers.extend([now_rule, now_text])

    chart = alt.layer(*layers).properties(height=285).configure_view(strokeOpacity=0).configure_axis(
        gridOpacity=.12, labelColor="#cfc5cf", titleColor="#d8cdd7"
    ).configure_legend(labelColor="#d8cdd7")
...
/* no floating glyph ornament under opportunity headers */
.we-cardhead::after { content:none; display:none; }
.we-recommend {
    background:
...
.we-recommend::after {
    ...
    background:
      repeating-linear-gradient(to bottom,
        rgba(255,157,211,.32) 0 6px,
        rgba(255,157,211,.32) 6px 9px,
        transparent 9px 13px),
      radial-gradient(circle at 50% 50%,rgba(255,130,200,.22),rgba(186,130,255,.08) 62%,transparent 64%) !important;
    box-shadow:0 0 26px rgba(255,117,195,.10) !important;
    opacity:.72 !important;
...
.we-recommend::before {
    ...
    font-size:2.65rem;
    opacity:.34;
    filter:grayscale(.15) saturate(.75) drop-shadow(0 0 6px rgba(0,0,0,.35));
    z-index:1;
    pointer-events:none;
}
.we-core {
...
.we-section-head::before {
    ...
    bottom:0;
    height:1px;
    background:linear-gradient(90deg,transparent,#ff8ecf 22%,#b89cff 50%,#8cecff 78%,transparent);
}
.we-section-head::after {
    content:"";
    position:absolute;
    ...
    opacity:.18;
    background:
      linear-gradient(rgba(150,236,255,.25) 1px,transparent 1px),
      linear-gradient(90deg,rgba(255,143,208,.20) 1px,transparent 1px);
    background-size:100% 7px,22px 100%;
    transform:perspective(70px) rotateX(58deg);
...
/* v3.5 COSMIC SUNSET: warm vaporwave, immersive art, minimal blue */
.stApp {
    background:
      radial-gradient(ellipse at 8% 10%, rgba(255,68,158,.20), transparent 28rem),
      radial-gradient(ellipse at 92% 18%, rgba(185,78,255,.16), transparent 30rem),
      radial-gradient(ellipse at 52% 56%, rgba(255,112,96,.08), transparent 34rem),
      radial-gradient(ellipse at 18% 82%, rgba(255,166,92,.07), transparent 28rem),
      linear-gradient(180deg,#020202 0%,#090408 44%,#030203 100%) !important;
}
.stApp::before {
    opacity:.36 !important;
    background:
      radial-gradient(circle at 14% 11%, rgba(255,224,239,.20) 0 1px, transparent 1.5px),
      radial-gradient(circle at 73% 23%, rgba(239,208,255,.17) 0 1px, transparent 1.5px),
      radial-gradient(circle at 42% 66%, rgba(255,194,181,.13) 0 1px, transparent 1.5px),
      radial-gradient(circle at 88% 78%, rgba(255,225,194,.10) 0 1px, transparent 1.5px) !important;
    background-size:109px 109px,157px 157px,199px 199px,241px 241px !important;
}
/* warm horizon: no cyan wash */
.stApp::after {
    opacity:.10 !important;
    background:
      linear-gradient(rgba(255,136,184,.24) 1px,transparent 1px),
      linear-gradient(90deg,rgba(192,110,255,.18) 1px,transparent 1px) !important;
}

/* Header becomes a cosmic vaporwave scene */
.we-hero {
    min-height:248px !important;
    isolation:isolate;
    background:
      radial-gradient(circle at 50% 52%,rgba(255,92,172,.12),transparent 13rem),
...
.we-hero::before {
    ...
    background:
      repeating-linear-gradient(to bottom,
        rgba(255,151,202,.82) 0 8px,
        rgba(255,151,202,.82) 8px 11px,
        transparent 11px 16px) !important;
    box-shadow:
      0 0 44px rgba(255,75,166,.22),
      0 0 90px rgba(194,92,255,.08) !important;
...
.we-hero::after {
    ...
    background:
      linear-gradient(rgba(255,132,190,.22) 1px,transparent 1px),
      linear-gradient(90deg,rgba(190,105,255,.17) 1px,transparent 1px) !important;
}
.we-title {
    z-index:5 !important;
    font-size:3.55rem !important;
    text-shadow:0 3px 28px rgba(0,0,0,.75),0 0 20px rgba(255,117,194,.10) !important;
}
.we-title .edge {
    background:linear-gradient(90deg,#ffd7e9,#ff8fc9,#c69bff,#ffb08c) !important;
    -webkit-background-clip:text !important;
    -webkit-text-fill-color:transparent !important;
    background-clip:text !important;
}
.we-sub { z-index:5 !important; }

/* Large cosmic imagery layered inside hero */
.we-cosmic-planet {
    position:absolute;
    width:132px;
    height:132px;
    ...
    opacity:.72;
    background:
      radial-gradient(circle at 32% 28%,rgba(255,222,204,.65),rgba(231,118,191,.34) 31%,rgba(129,69,172,.26) 64%,rgba(18,8,22,.12) 72%);
    box-shadow:inset -20px -12px 30px rgba(0,0,0,.26),0 0 34px rgba(223,100,196,.11);
}
.we-cosmic-planet::after {
    content:"";
    position:absolute;
    left:-34px; right:-34px; top:56px;
    height:17px;
    border:2px solid rgba(255,192,218,.32);
    border-radius:50%;
...
.we-orbit {
    ...
    left:-58px;
    top:24px;
    border:1px solid rgba(223,166,255,.18);
    border-radius:50%;
...
.we-orbit::after {
    ...
    right:31px;top:3px;
    background:#ff9ac9;
    box-shadow:0 0 12px rgba(255,126,190,.65);
}
.we-hero-palm {
    position:absolute;
    z-index:3;
    ...
    font-size:5.6rem;
    opacity:.44;
    filter:grayscale(.12) saturate(.55) brightness(.58);
    text-shadow:0 0 16px rgba(255,92,173,.08);
}
.we-hero-palm.left { left:-12px; transform:rotate(-8deg); }
.we-hero-palm.right { right:5px; transform:scaleX(-1) rotate(-7deg); opacity:.32; }

/* Top opportunities: remove glitter/stars entirely */
.we-section-title { position:relative; z-index:2; }
.we-section-title::before,
.we-section-title::after { content:none !important; display:none !important; }
.we-section-head::after {
    background:
      linear-gradient(rgba(255,139,188,.23) 1px,transparent 1px),
      linear-gradient(90deg,rgba(190,105,255,.18) 1px,transparent 1px) !important;
}
.we-section-head::before {
    background:linear-gradient(90deg,transparent,#ff78b9 22%,#c28cff 52%,#ffad82 80%,transparent) !important;
}
.we-section-title {
    color:#fff5fa !important;
    text-shadow:0 0 18px rgba(255,113,184,.10) !important;
}

/* Larger image-like artwork across expanded cards */
...
[data-testid="stExpander"] details[open]::after {
    ...
    top:74px !important;
    background:
      repeating-linear-gradient(to bottom,rgba(255,123,188,.13) 0 7px,transparent 7px 12px),
      radial-gradient(circle,rgba(255,103,177,.08),transparent 67%) !important;
    opacity:.62 !important;
}
[data-testid="stExpander"] details[open]::before {
    width:3px !important;
    background:linear-gradient(180deg,#ff69b0,#c079ff,#ffae79) !important;
    opacity:.78 !important;
}
.we-recommend::after {
    background:
      repeating-linear-gradient(to bottom,
        rgba(255,144,196,.40) 0 6px,
        rgba(255,144,196,.40) 6px 9px,
        transparent 9px 13px),
      radial-gradient(circle at 50% 50%,rgba(255,115,181,.26),rgba(186,105,255,.09) 62%,transparent 64%) !important;
}
.we-recommend::before {
...
/* Large decorative planets/palms built from pseudo-elements on major cards */
.we-range-card {
    position:relative;
}
.we-range-card::before {
    ...
    border-radius:50% !important;
    background:
      radial-gradient(circle at 35% 30%,rgba(255,194,211,.18),rgba(187,112,244,.08) 48%,transparent 70%) !important;
    border:1px solid rgba(255,183,219,.08);
}
.we-signal-strip::after {
    width:105px !important;height:105px !important;
    right:-38px !important;top:-48px !important;
    background:
      repeating-linear-gradient(to bottom,rgba(255,137,192,.12) 0 5px,transparent 5px 9px) !important;
}
.we-market-card::before {
    content:"";
    position:absolute;
    width:190px;height:190px;
    left:-96px;bottom:-105px;
    border-radius:50%;
    background:radial-gradient(circle at 60% 35%,rgba(255,170,127,.09),rgba(189,99,242,.05) 48%,transparent 70%);
    pointer-events:none;
}
.we-market-card::after {
    background:
      linear-gradient(rgba(255,129,185,.20) 1px,transparent 1px),
      linear-gradient(90deg,rgba(188,100,255,.15) 1px,transparent 1px) !important;
}
...
.we-recommend,.we-core,.we-range-card,.we-sourcebox,
.we-signal-strip,.we-market-card,.we-marketcell,.we-section-head {
    border-color:rgba(255,210,230,.13) !important;
}
.we-core:nth-child(1) { border-color:rgba(255,132,190,.16) !important; }
.we-core:nth-child(2) { border-color:rgba(194,135,255,.17) !important; }
.we-core:nth-child(3) { border-color:rgba(255,177,127,.14) !important; }
.we-range-band {
    background:#d39cff !important;
    box-shadow:0 0 16px rgba(196,128,255,.18) !important;
}
.we-section-mini::before {
    background:linear-gradient(90deg,#ff83bd,#bd89ff) !important;
}
.we-section-mini::after {
    background:linear-gradient(90deg,rgba(255,161,197,.19),transparent) !important;
}
.we-live {
    color:#ffbadb !important;
    background:rgba(255,94,173,.065) !important;
    border-color:rgba(255,142,196,.18) !important;
}
@media (max-width:760px) {
    .we-hero { min-height:220px !important; }
    .we-title { font-size:2.75rem !important; }
    .we-cosmic-planet { width:100px;height:100px;right:-28px;top:24px; }
    .we-cosmic-planet::after { top:42px; }
    .we-orbit { width:150px;height:64px;left:-50px; }
    .we-hero-palm { font-size:4.2rem; }
}


/* v3.6 WEATHER COSMOS: refined orbital depth + varied weather motifs */
.we-hero {
    position:relative;
    overflow:hidden;
    isolation:isolate;
...
.we-cosmic-planet {
    ...
    opacity:.92;
    background:
      radial-gradient(circle at 30% 25%,rgba(255,229,211,.90),rgba(238,141,193,.58) 30%,rgba(170,94,209,.48) 61%,rgba(53,23,64,.68) 100%);
    box-shadow:
      inset -24px -12px 30px rgba(34,7,39,.36),
      inset 12px 7px 20px rgba(255,255,255,.09),
      0 0 34px rgba(232,107,187,.14);
}
.we-ring-back,
.we-ring-front {
    position:absolute;
    ...
    right:5px;
    top:58px;
    border:3px solid rgba(255,193,218,.36);
    border-radius:50%;
...
.we-ring-front {
    ...
    opacity:.92;
    border-color:rgba(255,204,224,.46);
}
.we-orbit {
    position:absolute;
    width:200px;
    ...
    right:-7px;
    top:34px;
    border:1px solid rgba(214,166,255,.22);
    border-radius:50%;
...
.we-orbit::after {
    ...
    border-radius:50%;
    background:#ff9bc8;
    box-shadow:0 0 12px rgba(255,139,198,.65);
}
.we-orbit-2 {
    position:absolute;
    width:150px;
    ...
    right:18px;
    top:8px;
    border:1px dashed rgba(255,174,215,.13);
    border-radius:50%;
...
.we-orbit-2::after {
    ...
    border-radius:50%;
    background:#c79fff;
    box-shadow:0 0 9px rgba(196,152,255,.48);
}

/* Weather constellation around the hero */
.we-weather-icons {
    position:absolute;
    inset:0;
    z-index:2;
...
.we-weather-icon {
    position:absolute;
    opacity:.46;
    filter:drop-shadow(0 0 9px rgba(255,164,215,.10));
}
.we-weather-icon.sun { left:7%; top:24px; font-size:1.9rem; }
.we-weather-icon.cloud { left:13%; bottom:34px; font-size:1.65rem; opacity:.34; }
.we-weather-icon.snow { left:30%; top:23px; font-size:1.35rem; color:#f0eaff; opacity:.40; }
.we-weather-icon.rain { right:31%; bottom:34px; font-size:1.45rem; opacity:.34; }
.we-weather-icon.bolt { right:23%; top:20px; font-size:1.35rem; opacity:.38; }
.we-weather-icon.wind { left:31%; bottom:28px; font-size:1.35rem; opacity:.32; }

/* Weather visuals repeated throughout cards */
.we-cardhead::before {
    content:"☀︎  ❄︎  ☁︎  ↯";
    position:absolute;
    right:0;
    top:0;
    color:rgba(255,190,220,.16);
    font-size:.78rem;
    letter-spacing:.38rem;
    pointer-events:none;
}
.we-recommend .weather-corner {
    position:absolute;
    left:13px;
    bottom:9px;
    display:flex;
    gap:.42rem;
    z-index:1;
    color:rgba(255,202,226,.20);
    font-size:1rem;
    pointer-events:none;
}
.we-core:nth-child(1)::before {
    content:"☀︎";
    position:absolute;
    right:8px;
    top:6px;
    font-size:1.55rem;
    color:rgba(255,163,203,.16);
}
.we-core:nth-child(2)::before {
    content:"❄︎";
    position:absolute;
    right:8px;
    top:6px;
    font-size:1.45rem;
    color:rgba(211,183,255,.16);
}
.we-core:nth-child(3)::before {
    content:"☁︎";
    position:absolute;
    right:8px;
    top:6px;
    font-size:1.45rem;
    color:rgba(255,191,149,.14);
}
.we-range-card .wx-row {
    position:absolute;
    right:13px;
    bottom:8px;
    color:rgba(255,186,216,.14);
    font-size:.95rem;
    letter-spacing:.32rem;
    pointer-events:none;
}
.we-signal-strip::before {
    content:"☾";
    position:absolute;
    left:10px;
    top:8px;
    color:rgba(212,180,255,.14);
    font-size:1.25rem;
}
.we-market-card .wx-market {
    position:absolute;
    right:12px;
    top:9px;
    color:rgba(255,183,215,.13);
    font-size:1rem;
    letter-spacing:.25rem;
    pointer-events:none;
}

/* Make weather visuals feel art-directed, not like stickers */
.we-weather-icon,
.we-cardhead::before,
.we-recommend .weather-corner,
.we-range-card .wx-row,
...
@media (max-width:760px) {
    ...
    .we-ring-back,.we-ring-front {
        width:138px;
        height:39px;
        right:5px;
        top:51px;
    }
    .we-orbit { width:155px; height:72px; right:-5px; top:34px; }
    .we-orbit-2 { width:118px; height:118px; right:14px; top:13px; }
    .we-weather-icon.sun { left:5%; }
    .we-weather-icon.cloud { left:7%; }
    .we-weather-icon.rain { right:28%; }
}


/* v3.7 COHESIVE HERO
   The expanded-card striped circles become the visual grammar for the header. */

/* Hide the previous collection of independent hero decorations */
.we-cosmic-system,
.we-weather-icons,
.we-hero-palm {
...
.we-hero {
    ...
    overflow:hidden;
    isolation:isolate;
    padding:2.1rem 1.25rem 2.3rem !important;
    border-radius:28px !important;
    background:
      radial-gradient(ellipse at 50% 46%,rgba(255,105,179,.12),transparent 15rem),
      radial-gradient(ellipse at 76% 28%,rgba(187,112,255,.08),transparent 13rem),
      linear-gradient(145deg,rgba(255,255,255,.047),rgba(255,255,255,.012)) !important;
    border:1px solid rgba(255,208,229,.16) !important;
    box-shadow:
      inset 0 1px 0 rgba(255,255,255,.09),
      0 24px 70px rgba(0,0,0,.27) !important;
}

/* Large striped celestial disc, same language as the expanded cards */
.we-hero-disc-large {
    position:absolute;
    width:245px;
    height:245px;
    ...
    background:
      repeating-linear-gradient(
        to bottom,
        rgba(255,132,191,.30) 0 8px,
        rgba(255,132,191,.30) 8px 12px,
        transparent 12px 18px
      ),
      radial-gradient(circle at 38% 30%,rgba(255,190,211,.16),rgba(188,105,244,.07) 55%,transparent 72%);
    border:1px solid rgba(255,185,218,.12);
    box-shadow:0 0 42px rgba(255,89,169,.09);
}

/* Smaller companion disc, deliberately related rather than a random planet */
.we-hero-disc-small {
    position:absolute;
    width:112px;
    height:112px;
    ...
    background:
      repeating-linear-gradient(
        to bottom,
        rgba(205,151,255,.28) 0 5px,
        rgba(205,151,255,.28) 5px 8px,
        transparent 8px 12px
      ),
      radial-gradient(circle,rgba(216,176,255,.10),transparent 68%);
    border:1px solid rgba(218,178,255,.11);
}

/* Left-side balancing geometry */
.we-hero-glass-orbit {
    position:absolute;
    width:250px;
    height:105px;
    ...
    bottom:30px;
    z-index:1;
    border:1px solid rgba(255,158,207,.13);
    border-radius:50%;
    transform:rotate(-15deg);
}
.we-hero-glass-orbit::before {
    content:"";
    position:absolute;
    width:12px;
    height:12px;
    ...
    top:5px;
    border-radius:50%;
    background:rgba(255,157,205,.72);
    box-shadow:0 0 16px rgba(255,112,189,.35);
}
.we-hero-glass-orbit::after {
    content:"";
    position:absolute;
    width:82px;
    height:82px;
    ...
    top:9px;
    border-radius:50%;
    border:1px solid rgba(255,200,223,.08);
    background:radial-gradient(circle at 32% 27%,rgba(255,255,255,.12),rgba(255,149,205,.025) 55%,transparent 70%);
    backdrop-filter:blur(5px);
}

/* Controlled horizon fragment, not a full-screen grid */
.we-hero-horizon {
    position:absolute;
    left:8%;
    right:8%;
    ...
    opacity:.26;
    background:
      linear-gradient(rgba(255,135,191,.25) 1px,transparent 1px),
      linear-gradient(90deg,rgba(193,111,255,.18) 1px,transparent 1px);
    background-size:100% 15px,38px 100%;
    transform:perspective(170px) rotateX(60deg);
    transform-origin:bottom;
    mask-image:linear-gradient(to top,black,transparent 88%);
    -webkit-mask-image:linear-gradient(to top,black,transparent 88%);
}

/* One subtle weather/tropical signature, integrated rather than scattered */
.we-hero-signature {
    position:absolute;
    left:24px;
    top:24px;
    z-index:2;
    color:rgba(255,210,229,.25);
    font-size:.92rem;
    letter-spacing:.48rem;
    writing-mode:vertical-rl;
}
.we-hero-palm-silhouette {
    position:absolute;
    left:-18px;
    bottom:-17px;
    z-index:2;
...
/* Title becomes the unmistakable focal point */
.we-title {
    position:relative;
    z-index:6 !important;
    ...
    text-shadow:
      0 4px 34px rgba(0,0,0,.72),
      0 0 28px rgba(255,110,184,.10) !important;
}
.we-title .edge {
    background:linear-gradient(90deg,#ffe0ed 0%,#ff8fc8 34%,#c39aff 66%,#ffb08b 100%) !important;
    -webkit-background-clip:text !important;
    -webkit-text-fill-color:transparent !important;
    background-clip:text !important;
}
.we-sub {
    ...
    color:#d8cbd3 !important;
    background:rgba(6,4,7,.48) !important;
    border:1px solid rgba(255,215,232,.09) !important;
    box-shadow:inset 0 1px 0 rgba(255,255,255,.04);
}

/* tiny composition line under title, tied to the discs */
.we-hero-titleline {
    position:relative;
    z-index:6;
    width:132px;
    ...
    background:linear-gradient(90deg,transparent,#ff83bd,#c58eff,#ffad84,transparent);
    opacity:.68;
}
.we-hero-titleline::before,
.we-hero-titleline::after {
    content:"";
    position:absolute;
    top:-2px;
...
.we-hero-titleline::before { left:34px; }
.we-hero-titleline::after { right:34px; background:#ff9bc6; }

@media (max-width:760px) {
    .we-hero { min-height:280px !important; padding:1.8rem .8rem 2rem !important; }
    .we-title { font-size:3.15rem !important; }
    .we-hero-disc-large { width:190px;height:190px;right:-65px;top:-28px; }
    .we-hero-disc-small { width:88px;height:88px;right:95px;top:112px; }
    .we-hero-glass-orbit { width:190px;height:82px;left:-90px;bottom:34px; }
    .we-hero-palm-silhouette { font-size:5rem; }
    .we-hero-signature { left:14px;top:18px;font-size:.72rem; }
}
...
.we-localtime {
    ...
    padding:.42rem .68rem;
    border-radius:999px;
    border:1px solid rgba(255,198,225,.16);
    background:rgba(8,5,9,.62);
    color:#f7d7e7;
    font-size:.82rem;
    font-weight:780;
    backdrop-filter:blur(10px);
}
.we-localtime .clockdot {
    width:7px;height:7px;border-radius:50%;
    background:#ff8fc5;
    box-shadow:0 0 10px rgba(255,125,190,.42);
}
.we-intraday-head {
    display:flex;
    align-items:end;
...
<div class="we-hero">
  <div class="we-hero-disc-large"></div>
  <div class="we-hero-disc-small"></div>
  <div class="we-hero-glass-orbit"></div>
  <div class="we-hero-horizon"></div>
  <div class="we-hero-signature">☀︎ ❄︎ ☁︎ ↯</div>
  <div class="we-hero-palm-silhouette">🌴</div>
  <div class="we-title">Weather<span class="edge">Edge</span></div>
  <div class="we-hero-titleline"></div>
</div>
    ...
    with st.expander(expander_label, expanded=False):
        ...
        st.markdown(
            ...
            <div class="we-recommend">
              ...
              <div class="weather-corner">☀︎ ❄︎ ☁︎ ↯</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        local_now = datetime.now(ZoneInfo(r["tz_name"]))
        st.markdown(
            f"""<div class="we-localtime"><span class="clockdot"></span>
            Current time in {r['city']}: {local_now.strftime('%-I:%M %p')}
            <span style="opacity:.58;font-weight:600;">{local_now.tzname() or ''}</span></div>""",
            unsafe_allow_html=True,
        )
        ...
        st.markdown(
            f"""
            <div class="we-intraday-head">
              <div class="we-intraday-title">Temperature through the day</div>
              <div class="we-intraday-note">Observed station readings + latest NWS hourly forecast<br>all times shown in {r['city']} local time</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
        render_intraday_temperature_chart(
            r["lat"], r["lon"], r.get("station_id"), r["tz_name"], r["date"]
        )
