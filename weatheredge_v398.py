...
def latest_projection_chart(snapshot_df, observed_df, previous_observed_df, tz_name, target_date, city=None):
    ...
    # Clearly mark the exact current local date/time for the market city.
    if x_min <= now_local <= x_max:
        now_df = pd.DataFrame({"time": [now_local]})
        city_prefix = f"{city} • " if city else ""
        now_label = f"NOW • {city_prefix}{now_local.strftime('%b %-d, %Y • %-I:%M %p %Z')}"
    ...
    chart = alt.layer(*layers).resolve_scale(color="shared").properties(
        ...
        title=alt.TitleParams(
            ...
            subtitle="Solid = observed · dashed = last stored NWS forecast for each hour · cyan line/label = exact current local time",
            anchor="start",
        ),
    ).configure_axis(
...
def render_bet_forecast(city, contract_date):
    ...
    latest_chart, latest_key = latest_projection_chart(
        snapshot_df, observed, previous_observed, cfg["tz"], contract_date, city=city
    )
