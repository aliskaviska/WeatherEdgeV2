...
def recent_completed_day_adjustment(contract_date, hours_to_peak, calibration):
    ...
    try:
        ...
        # A large miss yesterday must matter, but it should not teleport today's
        # center several degrees away from both today's NWS forecast and today's
        # observations. Put most of that evidence into uncertainty, with a
        # meaningful but bounded directional center shift.
        if yesterday is not None and abs(yesterday) >= 3.0:
            minimum = min(2.0, abs(yesterday) * 0.40)
            if consistency >= 0.60 or n == 1:
                adjustment = math.copysign(max(abs(adjustment), minimum), yesterday)

        adjustment = max(-2.75, min(2.75, adjustment))

        recent_mae = sum(w * abs(e) for w, e in zip(weights, errors)) / max(wsum, 1e-9)
        if consistency < 0.60 and recent_mae >= 1.5:
            sigma_multiplier = min(1.60, 1.12 + 0.10 * recent_mae)
        elif recent_mae >= 3.0:
            sigma_multiplier = min(1.50, 1.12 + 0.08 * recent_mae)
    ...
    for m in markets:
        ...
        try:
            ...
            previous_day_prediction = previous_day_forecast_summary(
                city, cfg["tz"], d, calibration=calibration
            )
            previous_day_predicted = previous_day_predicted_high(
                city, cfg["tz"], d, calibration
            )
            if previous_day_predicted is None and previous_day_prediction is not None:
                previous_day_predicted = {
                    "predicted_high_f": float(previous_day_prediction["latest_f"]),
                    "snapshot_at": None,
                    "source": previous_day_prediction.get("source_label") or "stored NWS snapshots",
                }

            # Important future-date fallback:
            # If the market is for tomorrow, its "previous day" is today, which
            # is not yet a completed historical day and therefore may not exist
            # in calibration/snapshot history. Use the already-loaded live NWS
            # daily forecast for that previous calendar day.
            if previous_day_predicted is None:
                previous_day_date = d - timedelta(days=1)
                previous_day_nws_row = nws.get(previous_day_date, {}) or {}
                previous_day_nws_high = previous_day_nws_row.get("nws_high_f")
                if previous_day_nws_high is not None:
                    fallback_high = float(previous_day_nws_high)
                    previous_day_predicted = {
                        "predicted_high_f": fallback_high,
                        "snapshot_at": None,
                        "source": "current NWS daily forecast",
                    }
                    previous_day_prediction = {
                        "average_f": fallback_high,
                        "low_f": fallback_high,
                        "high_f": fallback_high,
                        "latest_f": fallback_high,
                        "n_snapshots": 1,
                        "source_label": "current NWS daily forecast",
                    }

            # Fallback for the chart: the calibration metadata contains one
            # representative near-peak NWS forecast for each completed day.
            # This guarantees that yesterday's predicted high remains visible
            # even when the exact "day-before" snapshot slice is missing.
            if previous_day_prediction is None:
                ...
                prior_match = next(
                    (
                        ...
                        if str(item.get("date")) == previous_day.isoformat()
                    ),
                    None,
                )
                if prior_match is not None:
                    prior_pred = float(prior_match["projected_high_f"])
                    previous_day_prediction = {
                        "average_f": prior_pred,
                        "low_f": prior_pred,
                        "high_f": prior_pred,
                        "latest_f": prior_pred,
                        "n_snapshots": 1,
                        "source_label": "near-peak historical NWS forecast",
                    }
                    previous_day_predicted = {
                        "predicted_high_f": prior_pred,
                        "snapshot_at": None,
                        "source": "near-peak historical NWS forecast",
                    }
        except Exception:
            observed_high = None
            observed_high_time = None
            ...
            # Even if one of the observation helpers failed, still try to retain
            # the previous day's predicted high from calibration metadata.
            previous_day_prediction = None
            try:
                previous_day_predicted = previous_day_predicted_high(
                    city, cfg["tz"], d, calibration
        ...
        if daily_high_locked:
            # Hard post-peak rule requested for conservative betting: once the
            # day is clearly past its hottest period, do not assign any chance to
            # a temperature above the highest value already observed.
            ...
            p_yes = nws_yes_probability(
                ...
                hours_to_peak=hours_to_peak,
                trajectory_adjustment_f=0.0,
                trajectory_sigma_multiplier=combined_sigma_multiplier,
                mean_override_f=adjusted_nws_center,
            )
        if p_yes is None:
            continue
        ...
        for side, ask, p_nws in side_data:
            ...
            rows.append({
                ...
                "nws_sigma_f": (
                    0.12
                    if daily_high_locked
                    else nws_sigma_f(hours_to_peak, calibration)
                    * combined_sigma_multiplier
                ),
...
def scan_live_market_universe():
    ...
    rows = []
    errors = []
    # Four workers keep NWS/GFS/Supabase I/O concurrent while Kalshi calls are
    # separately serialized/paced by get_json.
    max_workers = min(4, max(1, len(PRESETS)))
...
def previous_day_forecast_summary(city, tz_name, target_date, calibration=None):
    ...
    The dotted line and shaded range MUST come from the same set of stored
    snapshots that predicted the previous calendar day's high.

    For a completed previous day:
      use all stored snapshots up to that day's observed peak when known;
      otherwise use snapshots up to the end of that day.

    For a previous day that is still today:
      use all snapshots captured up to now.

    Returns min/max/latest/average from one internally consistent snapshot set.
    """
    ...
    if previous_day < now_local.date() and peak_minutes is not None:
        cutoff = (
            pd.Timestamp(
                datetime.combine(
                    previous_day,
    ...
    elif previous_day < now_local.date():
        cutoff = pd.Timestamp(
            datetime.combine(
                previous_day,
                datetime.max.time(),
    ...
    eligible = projected[
        projected["snapshot_at"].apply(
            lambda x: pd.Timestamp(x).tz_convert(tz) <= cutoff
        )
    ].copy()

    if eligible.empty:
        # If all stored snapshots happen to be after the preferred cutoff,
        # fall back to the earliest available set rather than fabricate a value.
        eligible = projected.copy()
    ...
    # Defensive invariant: the latest value must mathematically lie inside the
    # displayed range. This should already be true because all three values use
    # the same dataframe, but keep the invariant explicit.
    low = min(low, latest)
    ...
    return {
        ...
        "source_label": "same snapshot set through previous-day peak",
    }
...
def latest_projection_chart(snapshot_df, observed_df, previous_observed_df, tz_name, target_date):
    ...
    # Add direct labels for the previous day's observed high and the last
    # stored NWS-predicted high for that previous day.
    try:
        ...
        if previous_observed_df is not None and not previous_observed_df.empty:
            ...
            prev_obs["time"] = pd.to_datetime(
                prev_obs["time"], utc=True, errors="coerce"
            ).dt.tz_convert(tz)
            prev_obs["temp_f"] = pd.to_numeric(
                prev_obs["temp_f"], errors="coerce"
            )
            prev_obs = prev_obs.dropna(subset=["time", "temp_f"])
            prev_obs = prev_obs[prev_obs["time"].dt.date == prev_day]
            if not prev_obs.empty:
                high_val = float(prev_obs["temp_f"].max())
                high_rows = prev_obs[
                    prev_obs["temp_f"] >= high_val - 0.05
                ].sort_values("time")
                high_point = high_rows.iloc[0]
                prev_label_rows.append({
                    "time": high_point["time"],
                    "temp_f": high_val,
                    "label": f"Prev observed high {high_val:.1f}°",
                    "kind": "Previous observed high",
                })

        prev_snap = snapshot_df.copy()
        prev_snap["valid_local"] = prev_snap["valid_at"].dt.tz_convert(tz)
        prev_snap["snapshot_local"] = prev_snap["snapshot_at"].dt.tz_convert(tz)
        prev_snap = prev_snap[
            prev_snap["valid_local"].dt.date == prev_day
        ].copy()
        ...
        if not prev_snap.empty:
            ...
            # Prefer the last forecast available before the observed previous-day
            # high; otherwise use the latest stored forecast for that day.
            chosen_prev = None
            if previous_observed_df is not None and not previous_observed_df.empty:
                try:
                    prev_obs2 = previous_observed_df.copy()
                    prev_obs2["time"] = pd.to_datetime(
                        prev_obs2["time"], utc=True, errors="coerce"
                    ).dt.tz_convert(tz)
                    prev_obs2["temp_f"] = pd.to_numeric(
                        prev_obs2["temp_f"], errors="coerce"
                    )
                    prev_obs2 = prev_obs2.dropna(subset=["time", "temp_f"])
                    prev_obs2 = prev_obs2[prev_obs2["time"].dt.date == prev_day]
                    if not prev_obs2.empty:
                        high_val2 = float(prev_obs2["temp_f"].max())
                        peak_time = (
                            prev_obs2[prev_obs2["temp_f"] >= high_val2 - 0.05]
                            .sort_values("time")
                            .iloc[0]["time"]
                        )
                        eligible_prev = projected_prev[
                            projected_prev["snapshot_at"].apply(
                                lambda x: pd.Timestamp(x).tz_convert(tz) <= peak_time
                            )
                        ]
                        if not eligible_prev.empty:
                            chosen_prev = eligible_prev.iloc[-1]
                except Exception:
                    chosen_prev = None

            if chosen_prev is None and not projected_prev.empty:
                chosen_prev = projected_prev.iloc[-1]

            if chosen_prev is not None:
                pred_val = float(chosen_prev["projected_high_f"])
                pred_time = pd.Timestamp(chosen_prev["snapshot_at"]).tz_convert(tz)
                prev_label_rows.append({
                    "time": pred_time,
                    "temp_f": pred_val,
                    "label": f"Prev NWS high {pred_val:.1f}°",
                    "kind": "Previous NWS predicted high",
                })

        if prev_label_rows:
            prev_label_df = pd.DataFrame(prev_label_rows)
            prev_scale = alt.Scale(
                domain=["Previous observed high", "Previous NWS predicted high"],
                range=["#FF6B5E", "#36C2FF"],
            )
            layers.append(
                alt.Chart(prev_label_df)
                .mark_point(filled=True, size=90)
                .encode(
                    x=alt.X("time:T"),
                    y=alt.Y("temp_f:Q"),
                    color=alt.Color("kind:N", scale=prev_scale, legend=None),
                    tooltip=[
                        alt.Tooltip("kind:N", title="Previous day"),
    ...
    chart = (
        ...
        .encode(
            x=alt.X(
                ...
                axis=alt.Axis(format="%b %-d, %-I:%M %p", labelAngle=-30, tickCount=7, grid=True, gridOpacity=0.42, tickSize=7),
            ),
            y=alt.Y(
                "predicted_high_f:Q",
...
with st.sidebar:
    ...
    st.caption(
        ...
        "Fixed safety rules are always on: settlement-source rounding ambiguity is "
        "excluded, and WeatherEdge will not recommend any side Kalshi prices at "
        "5% implied probability or less."
    )
...
# Fixed outlier-safety rule: do not recommend a side that Kalshi itself is
# pricing at 5% implied probability or less. Very-low-priced outcomes can reflect
# stale, odd, or structurally unusual markets, so exclude them from Top Bets.
kalshi_market_floor_mask = df["ask"].notna() & (df["ask"] > 0.05)
...
def contract_status(row):
    ...
    if float(row.get("ask", 0.0) or 0.0) <= 0.05:
        reasons.append("Kalshi market probability is 5% or less")
...
if main_view == "Best Bets":
    ...
    if not available_cities:
        ...
        if (
            ...
            st.caption(
                "Previous-day observed high is available, but no stored NWS forecast "
                "snapshot for that day's high was found yet. Historical forecast data "
                "will appear after the snapshot/calibration cache refreshes."
            )
