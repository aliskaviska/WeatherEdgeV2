...
def latest_projection_chart(snapshot_df, observed_df, previous_observed_df, tz_name, target_date):
    """Two-series chart: continuous observations vs newest stored NWS hourly forecast."""
    ...
    # Keep ALL valid times from the latest pull for the target date, including hours now in the past.
    latest = latest[latest["time"].dt.date == target_date].sort_values("time")
    if latest.empty:
        return None, latest_key
    forecast = latest[["time", "temp_f"]].copy()
    ...
    observations=pd.concat(obs_parts,ignore_index=True).sort_values("time") if obs_parts else pd.DataFrame(columns=["time","temp_f"])
    if not observations.empty:
        start_time=datetime.combine(target_date-timedelta(days=1), datetime.min.time(), tzinfo=tz)+timedelta(hours=18)
        end_time=datetime.combine(target_date+timedelta(days=1), datetime.min.time(), tzinfo=tz)
        observations=observations[(observations["time"]>=start_time)&(observations["time"]<end_time)].copy()
    ...
    layers=[]
    if not observations.empty:
        layers.append(alt.Chart(observations).mark_line(point=alt.OverlayMarkDef(filled=True,size=42),strokeWidth=3).encode(
            x=alt.X("time:T",title="Local time",axis=alt.Axis(format="%b %-d, %-I %p",labelAngle=-30,tickCount=7,grid=True,tickSize=7)),
            y=alt.Y("temp_f:Q",title="Temperature (°F)",scale=alt.Scale(zero=False),axis=alt.Axis(tickCount=7,grid=True,tickSize=7)),
            color=alt.Color("Series:N",scale=alt.Scale(domain=domain,range=rng),legend=alt.Legend(title=None)),
            tooltip=[alt.Tooltip("time:T",title="Observed time",format="%b %-d, %-I:%M %p"),alt.Tooltip("temp_f:Q",title="Observed",format=".1f")]))
    layers.append(alt.Chart(forecast).mark_line(point=alt.OverlayMarkDef(filled=True,size=42),strokeWidth=3,strokeDash=[7,4]).encode(
        x=alt.X("time:T",title="Local time",axis=alt.Axis(format="%b %-d, %-I %p",labelAngle=-30,tickCount=7,grid=True,tickSize=7)),
        y=alt.Y("temp_f:Q",title="Temperature (°F)",scale=alt.Scale(zero=False),axis=alt.Axis(tickCount=7,grid=True,tickSize=7)),
        color=alt.Color("Series:N",scale=alt.Scale(domain=domain,range=rng),legend=alt.Legend(title=None)),
        tooltip=[alt.Tooltip("time:T",title="Forecast valid time",format="%b %-d, %-I:%M %p"),alt.Tooltip("temp_f:Q",title="NWS prediction",format=".1f")]))

    chart=alt.layer(*layers).resolve_scale(color="shared").properties(height=330,background="#11121B",
        title=alt.TitleParams(text="Observed vs NWS prediction",subtitle="Observed is one continuous series · dashed NWS line keeps past valid times from the latest stored forecast",anchor="start")
    ).configure_axis(labelFontSize=13,titleFontSize=15,labelColor="#F1EDF7",titleColor="#FAF7FF",gridColor="#777185",gridOpacity=.25,
                     domainColor="#AAA3B5",tickColor="#AAA3B5").configure_legend(
        orient="top",direction="horizontal",title=None,labelFontSize=14,labelColor="#F1EDF7",symbolSize=170
    ).configure_title(fontSize=19,subtitleFontSize=13,color="#FAF7FF",subtitleColor="#D4CEDD").configure_view(strokeWidth=0)
