...
def max_projection_history_chart(snapshot_df, tz_name):
    ...
    chart = (
        alt.Chart(history)
        .mark_line(point=alt.OverlayMarkDef(filled=True, size=54), strokeWidth=3)
        .encode(
            x=alt.X(
                "snapshot_time:T",
...
def render_forecast_monitor(default_city=None):
    """Two compact figures driven by the Supabase snapshot collector."""
    st.subheader("Forecast monitor")
    st.caption(
        "Stored NWS projections are compared with settlement-station observations, "
        "then the forecast's predicted daily high is tracked from snapshot to snapshot."
    )

    initial_city = default_city if default_city in PRESETS else list(PRESETS.keys())[0]
    city_index = list(PRESETS.keys()).index(initial_city)
    ...
    with c1:
        chart_city = st.selectbox(
            ...
            list(PRESETS.keys()),
            index=city_index,
            key="forecast_monitor_city",
        )
    cfg = PRESETS[chart_city]
    local_today = datetime.now(ZoneInfo(cfg["tz"])).date()
    with c2:
        chart_date = st.date_input(
            "Forecast date",
            value=local_today,
            key="forecast_monitor_date",
        )

    rows, snapshot_error = get_snapshot_rows(chart_city, chart_date)
    if snapshot_error:
        st.info(snapshot_error)
        return
    if not rows:
        st.info(f"No stored forecast snapshots yet for {chart_city} on {chart_date:%b %-d}.")
        return
    ...
    observed = get_station_observations(cfg.get("station_id"), cfg["tz"], chart_date)
    latest_chart, latest_key = latest_projection_chart(snapshot_df, observed, cfg["tz"], chart_date)
    ...
    if history_chart is not None:
        ...
        st.info("A forecast-history line will appear after more than one snapshot is stored.")
