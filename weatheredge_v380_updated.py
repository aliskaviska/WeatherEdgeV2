...
def get_snapshot_rows(city, contract_date):
    ...
    if not url or not key:
        return [], (
            "Snapshot charts need SUPABASE_URL and either "
            "SUPABASE_SERVICE_ROLE_KEY or SUPABASE_ANON_KEY in this app's secrets."
        )
...
def max_projection_history_chart(snapshot_df, tz_name):
    ...
    chart = (
        ...
        .configure_axis(labelFontSize=12, titleFontSize=12, gridOpacity=0.16)
        .configure_view(strokeWidth=0)
    )
...
st.markdown("""
<style>
:root { --ink:#F7F4FF; --muted:#B9B5C9; --violet:#B79CFF; --pink:#F4A7D3; --cyan:#7DE7F2; }
.stApp {
  background:
    radial-gradient(circle at 12% 5%, rgba(183,156,255,.16), transparent 30rem),
    radial-gradient(circle at 90% 18%, rgba(125,231,242,.10), transparent 28rem),
    linear-gradient(160deg, #0B0A12 0%, #11101A 48%, #0B1117 100%);
  color: var(--ink);
}
.block-container {max-width: 900px; padding-top: 1.25rem; padding-bottom: 3rem;}
[data-testid="stMetric"] {
  background: linear-gradient(145deg, rgba(255,255,255,.065), rgba(183,156,255,.035));
  border: 1px solid rgba(183,156,255,.16); padding: 12px; border-radius: 16px;
  box-shadow: 0 10px 34px rgba(0,0,0,.14);
}
[data-testid="stMetricValue"] {color:#F7F4FF;}
div[data-testid="stVerticalBlock"] > div:has(> div[data-testid="stHorizontalBlock"]) {gap:.7rem;}
[data-testid="stRadio"] > div {gap:.45rem;}
[data-testid="stRadio"] label {
  background:rgba(255,255,255,.045); border:1px solid rgba(183,156,255,.18);
  border-radius:999px; padding:.28rem .62rem; transition:.15s ease;
}
[data-testid="stRadio"] label:hover {border-color:rgba(125,231,242,.55); background:rgba(125,231,242,.07);}
.small-note {font-size:.9rem; opacity:.75;}
.card-title {font-size:1.55rem; font-weight:760; margin-bottom:.15rem; letter-spacing:-.02em;}
.card-sub {font-size:1rem; color:#C9C4D8; margin-bottom:.75rem;}
.section-kicker {font-size:.72rem; letter-spacing:.16em; color:#B79CFF; font-weight:750; margin:1.25rem 0 .55rem;}
.bet-shell {padding:.25rem 0 .5rem;}
hr {border-color:rgba(183,156,255,.14)!important;}
</style>
...
if not qualified.empty:
    ...
    # Keep the weather evidence beside the recommendation it supports.
    ...
    st.link_button("Open exact market on Kalshi", r["kalshi_event_url"], use_container_width=True)
    st.caption(
