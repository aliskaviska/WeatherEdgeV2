...
def previous_day_predicted_high(city, tz_name, target_date, calibration=None):
    """
    Return the NWS-predicted high FOR the previous calendar day.

    Example: on Aug 12, this returns what NWS predicted Aug 11's final high
    would be. Prefer the most recent stored forecast before Aug 11's observed
    hottest point; otherwise use the latest stored forecast for Aug 11.
    """
    ...
    try:
        rows, err = get_snapshot_rows(city, None)
        snap, norm_err = normalize_snapshot_rows(rows)
        if not err and not norm_err and not snap.empty:
            work = snap.copy()
            work["valid_local"] = work["valid_at"].dt.tz_convert(tz)
            ...
            if not target.empty:
                ...
                if not projected.empty:
                    ...
                    if chosen is None:
                        chosen = projected.iloc[-1]

                    return {
                        "predicted_high_f": float(chosen["projected_high_f"]),
                        "snapshot_at": chosen["snapshot_at"],
                        "source": "stored NWS snapshot",
                    }
    except Exception:
        pass
    ...
    try:
        ...
        if match is not None:
            return {
                "predicted_high_f": float(match["projected_high_f"]),
                "snapshot_at": None,
                "source": "historical calibration record",
            }
    except Exception:
        pass
