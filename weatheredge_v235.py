...
st.markdown("""
<div class="we-topbar">
  <div class="we-badge">⚡ WEATHEREDGE v2.3.5 • VISUAL GFS RANGE</div>
...
for rank, (_, r) in enumerate(qualified.iterrows(), start=1):
    with st.container(border=True):
        ...
        # Visual comparison strip: GFS 10th–90th range with median,
        # projected daily high, and observed high markers.
        ...
        if low_v is not None and high_v is not None:
            ...
            axis_min = math.floor(min(visible_vals) - 2)
            axis_max = math.ceil(max(visible_vals) + 2)
            ...
            if median_pos is not None:
                marker_html += (
                    ...
                    f'<div class="we-range-label" style="left:{median_pos:.2f}%">Med {median_v:.1f}°</div>'
                )
            ...
            st.markdown(
                ...
                <div class="we-range-card">
                  <div class="we-range-head">
                    ...
                    <div class="we-range-spread">80% range width: {spread:.1f}°F</div>
                  </div>
                  <div class="we-range-axis">
                    <div class="we-range-track"></div>
                    <div class="we-range-band" style="left:{band_left:.2f}%;width:{band_width:.2f}%"></div>
                    {marker_html}
                  </div>
                  <div style="display:flex;justify-content:space-between;color:#7f8c9b;font-size:.70rem;margin-top:.15rem;">
                    <span>{axis_min}°</span><span>{axis_max}°</span>
                  </div>
                  <div class="we-range-legend">
                    <span><i class="we-dot band"></i>GFS 10–90%: {low_v:.1f}–{high_v:.1f}°F</span>
                    <span><i class="we-dot median"></i>GFS median: {median_v:.1f}°F</span>
                    <span><i class="we-dot nws"></i>Projected daily high: {nws_v:.0f}°F</span>
                    <span><i class="we-dot obs"></i>Observed high: {"—" if obs_v is None else f"{obs_v:.0f}°F"}</span>
                  </div>
                </div>
                """,
                unsafe_allow_html=True,
