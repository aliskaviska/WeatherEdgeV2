...
def forecast_range_summary_chart(row, show_bet_overlay=False):
    ...
    values = [v for v in (low, high, median, projected, observed, bet_low, bet_high) if v is not None]
    if not values:
        return None
    ...
    # Recommended Kalshi contract interval. Keep this addition isolated to this
    # summary chart so the Observed vs NWS prediction figure is unchanged.
    ...
    if show_bet_overlay and band_lo is not None and band_hi is not None and bet_side in ("YES", "NO"):
        bet_df = pd.DataFrame([{
            "low": band_lo,
            "high": band_hi,
            "lane": "Daily high",
            "mid": (band_lo + band_hi) / 2,
            "label": f"BET {bet_side}",
        }])
        layers.append(alt.Chart(bet_df).mark_rect(opacity=.18, color="#FFD166").encode(
        ...
        layers.append(alt.Chart(bet_df).mark_text(
            dy=37, fontSize=13, fontWeight="bold", color="#FFD166"
        ).encode(
            x=alt.X("mid:Q", scale=alt.Scale(domain=[domain_min, domain_max])),
            y=alt.Y("lane:N", axis=None),
            text="label:N"
        ))
    ...
    return alt.layer(*layers).resolve_scale(color="shared").properties(
        ...
        title=alt.TitleParams(text="Forecast range & daily highs", subtitle="GFS range/median, NWS projected high, observed high, and recommended bet interval", anchor="start")
...
st.caption("Best Bets, suggested opportunities, and a city-by-city weather market explorer.")
...
# Suggested bets are near-misses worth inspecting, but they are deliberately kept
# separate from the stricter Best Bets list.
suggested = df[~df.index.isin(qualified_all.index)].copy()
suggested = suggested[
    (suggested["nws_support"] == True)
    & (
        (suggested["conservative_prob"] >= max(0.55, min_nws_chance - 0.10))
        | (suggested["conservative_edge"] >= min_gap)
    )
].sort_values(
    ["bet_quality_score", "conservative_prob", "conservative_edge"],
    ascending=[False, False, False],
).head(max(top_n, 5))


def contract_status(row):
    """Explain whether a contract is a Top Bet, Suggested Bet, or neither."""
    ...
    suggestion_match = suggested[
        (suggested["market_ticker"] == ticker) & (suggested["side"] == side)
    ]
    if not suggestion_match.empty:
        return "SUGGESTED BET", "suggested"
    ...
    return "NOT CURRENTLY RECOMMENDED" + (f" · {' · '.join(reasons)}" if reasons else ""), "other"
...
def render_contract_detail(r, show_weather=True):
    ...
    on_watchlist = contract_key in watchlist
    watch_label = "Remove from My Watchlist" if on_watchlist else "Add to My Watchlist"
    if st.button(
        ("★ " if on_watchlist else "☆ ") + watch_label,
        key=f"watch_toggle_{r['market_ticker']}_{r['side']}",
        use_container_width=True,
    ):
        if on_watchlist:
            watchlist = [k for k in watchlist if k != contract_key]
        else:
            watchlist.append(contract_key)
...
main_view = st.radio(
    "Main section",
    ["Best Bets", "Suggested Bets", "My Watchlist", "City Explorer"],
    horizontal=True,
    key="main_view",
...
if main_view == "Best Bets":
    ...
    st.caption("Only contracts that pass your active NWS-chance and Weather Edge safety filters.")
    ...
    if qualified.empty:
        ...
        selected = st.radio(
            ...
            horizontal=True,
            label_visibility="collapsed",
            key="best_bet_selector",
        )
        r = best_rows[best_labels.index(selected)]
        render_contract_detail(r, show_weather=False)

        if st.button("Explore this bet in City Explorer →", use_container_width=True):
...
elif main_view == "Suggested Bets":
    st.subheader("Suggested Bets")
    st.caption(
        "Interesting near-misses that do not currently qualify as Best Bets. "
        "Treat these as contracts to inspect, not as safety-filtered recommendations."
    )
    if suggested.empty:
        st.info("No suggested near-miss contracts right now.")
    else:
        suggested_rows = [r for _, r in suggested.iterrows()]
        suggested_labels = [
            f"{r['city']} · {r['side']} · {r['conservative_prob']*100:.0f}% NWS · {r['bet_quality_score']:.0f}/100"
        ...
        selected = st.radio(
            ...
            horizontal=False,
            label_visibility="collapsed",
            key="suggested_bet_selector",
        )
        r = suggested_rows[suggested_labels.index(selected)]
        render_contract_detail(r, show_weather=False)
...
elif main_view == "My Watchlist":
    st.subheader("My Watchlist")
    st.caption("Contracts you saved for quick follow-up. Watchlist items use live WeatherEdge values when the market is still open.")
    watched_keys = list(st.session_state.get("watchlist", []))
    if not watched_keys:
        st.info("Your watchlist is empty. Add contracts from Best Bets, Suggested Bets, or City Explorer.")
    else:
        live_lookup = {f"{row['market_ticker']}|{row['side']}": row for _, row in df.iterrows()}
        live_keys = [k for k in watched_keys if k in live_lookup]
        unavailable = [k for k in watched_keys if k not in live_lookup]
        if live_keys:
            watch_rows = [live_lookup[k] for k in live_keys]
            watch_labels = [
                f"{r['city']} · {r['market_subtitle']} · {r['side']} · "
                f"NWS {r['conservative_prob']*100:.0f}% · Edge {r['conservative_edge']*100:+.0f} pp"
                for r in watch_rows
            ]
            selected_watch = st.selectbox("Watchlist contract", watch_labels, label_visibility="collapsed")
            r = watch_rows[watch_labels.index(selected_watch)]
            render_contract_detail(r, show_weather=False)
            if st.button("Explore this watchlist bet in City Explorer →", use_container_width=True):
                st.session_state.explorer_city = r["city"]
                st.session_state.explorer_contract = f"{r['market_ticker']}|{r['side']}"
                st.session_state.main_view = "City Explorer"
                st.rerun()
        if unavailable:
            st.caption(f"{len(unavailable)} saved contract(s) are no longer in the current open-market scan.")
    ...
    if not available_cities:
        ...
        st.caption(
            ...
            "Top Bets and Suggested Bets are labeled automatically."
        )
        ...
        for row in contract_rows:
            ...
            badge = "★ " if kind == "best" else ("◇ " if kind == "suggested" else "")
