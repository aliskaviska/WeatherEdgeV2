...
def canonical_previous_day_nws_high_from_snapshots(
    snapshot_df,
    tz_name,
    target_date,
    observed_df=None,
    ...
    """
    Return the canonical NWS forecast history for the previous day's DAILY HIGH.

    Definition:
      • previous day = target_date - 1 in the city's local timezone
      • one projected high per stored NWS snapshot = max temperature forecast
        for that local calendar day
      • a snapshot is valid only if it was captured no later than the day's
        peak and its forecast horizon still covered that peak
      • the displayed previous-day predicted high is the projected daily high
        from the LAST such valid snapshot

    This prevents late-evening residual-hour forecasts from being mislabeled as
    the day's high, and gives every chart/legend one source of truth.
    """
    ...
    work["snapshot_at"] = pd.to_datetime(
        work["snapshot_at"], utc=True, errors="coerce"
    )
    work["valid_at"] = pd.to_datetime(
        work["valid_at"], utc=True, errors="coerce"
    )
    work["temp_f"] = pd.to_numeric(work["temp_f"], errors="coerce")
    work = work.dropna(subset=["snapshot_at", "valid_at", "temp_f"]).copy()
    if work.empty:
        return None
    ...
    # Only forecast hours belonging to the previous local calendar day.
    target = work[work["valid_local"].dt.date == previous_day].copy()
    if target.empty:
        return None

    # Determine when the previous day's actual maximum occurred. This is the
    # cleanest cutoff for "last forecast that was still genuinely predicting
    # the day's high."
    peak_dt = None
    if observed_df is not None and not observed_df.empty:
        try:
            ...
            elif "time_local" not in obs.columns:
                obs["time_local"] = pd.NaT
            obs["temp_f"] = pd.to_numeric(obs["temp_f"], errors="coerce")
            obs = obs.dropna(subset=["time_local", "temp_f"])
            obs = obs[obs["time_local"].dt.date == previous_day].sort_values(
                "time_local"
            )
            if not obs.empty:
                observed_high = float(obs["temp_f"].max())
                tied = obs[obs["temp_f"] >= observed_high - 0.15]
                if not tied.empty:
                    peak_dt = pd.Timestamp(tied.iloc[0]["time_local"])
        except Exception:
            peak_dt = None

    # Secondary peak-time source from calibration metadata.
    if peak_dt is None:
        try:
            peak_minutes = (
                (calibration or {})
    ...
    projected = (
        ...
        .agg(
            projected_high_f=("temp_f", "max"),
            snapshot_at=("snapshot_at", "min"),
            snapshot_local=("snapshot_local", "min"),
            valid_min=("valid_local", "min"),
            valid_max=("valid_local", "max"),
        )
    ...
    if peak_dt is not None:
        eligible = projected[
            (projected["snapshot_local"] <= peak_dt)
            & (projected["valid_min"] <= peak_dt)
            & (projected["valid_max"] >= peak_dt)
        ].copy()
    else:
        # If the exact peak is unavailable, avoid post-evening residual forecasts.
        cutoff = pd.Timestamp(
            datetime.combine(
                previous_day,
                datetime.min.time(),
        ...
        eligible = projected[
            projected["snapshot_local"] <= cutoff
        ].copy()

    if eligible.empty:
        # Do NOT use a late residual forecast. Use the latest snapshot that still
        # contains a substantial daytime window for the previous day.
        daytime_start = pd.Timestamp(
            datetime.combine(
                previous_day,
                datetime.min.time(),
                tzinfo=tz,
        ...
        eligible = projected[
            (projected["valid_min"] <= daytime_start)
            & (projected["valid_max"] >= daytime_end)
        ].copy()

    if eligible.empty:
        return None

    # This is the exact requested value: LAST valid NWS prediction for that
    # previous day's daily maximum.
    last = eligible.sort_values("snapshot_at").iloc[-1]
    latest = float(last["projected_high_f"])

    values = eligible["projected_high_f"].astype(float)
    low = float(values.min())
    high = float(values.max())

    # The last value and the range are guaranteed to come from the same set.
    low = min(low, latest)
    ...
    return {
        ...
        "latest_snapshot_at": last["snapshot_at"],
        "latest_snapshot_local": last["snapshot_local"],
        "snapshot_key": last["snapshot_key"],
        "peak_time_local": peak_dt,
        "source_label": "last valid stored NWS forecast for previous-day daily high",
    }
...
def previous_day_forecast_summary(city, tz_name, target_date, calibration=None):
    ...
    The single displayed value is the LAST valid NWS prediction for the highest
    temperature on the previous local calendar day.
    """
    snap, norm_err = get_normalized_snapshot_rows(city)
    if norm_err or snap is None or snap.empty:
        return None
    ...
    try:
        ...
        if station_id:
            observed = get_station_observations(
                station_id,
                tz_name,
    ...
    return canonical_previous_day_nws_high_from_snapshots(
        snap,
        tz_name,
        target_date,
...
def human_nws_forecast_url(city):
    """
    Human-readable NWS point forecast page for the selected city.
    Keep API endpoints internal; this is only for the user-facing source button.
    """
    cfg = PRESETS.get(city) or {}
    lat = cfg.get("lat")
    lon = cfg.get("lon")
...
def human_observed_temperatures_url(city):
    """
    Human-readable NWS observation history page for the configured settlement station.
    """
    cfg = PRESETS.get(city) or {}
    station_id = cfg.get("station_id")
    if not station_id:
    ...
    # NWS observation history endpoint is readable and station-specific.
    return (
...
def hydrate_city_explorer_row(row):
    ...
    try:
        ...
        # User-facing source links are rebuilt from the SELECTED city every time.
        # Do not expose raw API URLs or inherit stale URLs from another market row.
        _selected_city = str(row.get("city"))
        _forecast_link = human_nws_forecast_url(_selected_city)
        _observed_link = human_observed_temperatures_url(_selected_city)
        if _forecast_link:
            out["nws_forecast_url"] = _forecast_link
        if _observed_link:
            out["observed_data_url"] = _observed_link
        ...
        # Reconcile previous-day history from the canonical snapshot calculation
        # whenever the cached lazy path did not provide it. This prevents a stale
        # "missing history" cache entry from disagreeing with the chart below.
        if out.get("previous_day_predicted_high_f") is None:
            try:
                _snap, _snap_err = get_normalized_snapshot_rows(
                    str(row.get("city"))
                )
                _prev_obs = get_station_observations(
                    PRESETS[str(row.get("city"))].get("station_id"),
                    PRESETS[str(row.get("city"))]["tz"],
                    row.get("date") - timedelta(days=1),
                )
                _canonical = canonical_previous_day_nws_high_from_snapshots(
                    _snap,
                    PRESETS[str(row.get("city"))]["tz"],
                    row.get("date"),
                    observed_df=_prev_obs,
                    calibration=None,
                )
                if _canonical is not None:
                    _latest = float(_canonical["latest_f"])
                    out["previous_day_prediction_avg_f"] = _canonical.get("average_f")
                    out["previous_day_prediction_low_f"] = float(_canonical["low_f"])
                    out["previous_day_prediction_high_f"] = float(_canonical["high_f"])
                    out["previous_day_prediction_latest_f"] = _latest
                    out["previous_day_prediction_n"] = int(
                        _canonical.get("n_snapshots") or 0
                    )
                    out["previous_day_prediction_source"] = _canonical.get(
                        "source_label"
                    )
                    out["previous_day_predicted_high_f"] = _latest
                    out["previous_day_predicted_high_source"] = _canonical.get(
                        "source_label"
                    )
                    out["previous_day_prediction_consistent"] = True
            except Exception:
                pass

        # DISPLAY SOURCE OF TRUTH: derive previous-day predicted high from the
        # exact stitched trajectory used by the Observed vs NWS chart below.
        # If that figure can reconstruct yesterday, every other City Explorer
        # component must show the same value.
        try:
            _snap, _snap_err = get_normalized_snapshot_rows(
                str(row.get("city"))
            )
            if not _snap_err and _snap is not None and not _snap.empty:
                _stitched_prev = previous_day_high_from_stitched_trajectory(
                    _snap,
                    PRESETS[str(row.get("city"))]["tz"],
                    row.get("date"),
                )
                if _stitched_prev is not None:
                    _latest = float(_stitched_prev["latest_f"])
                    out["previous_day_predicted_high_f"] = _latest
                    out["previous_day_prediction_latest_f"] = _latest
                    out["previous_day_predicted_high_source"] = (
                        _stitched_prev.get("source_label")
                    )
                    out["previous_day_prediction_source"] = (
                        _stitched_prev.get("source_label")
                    )

                    # Preserve the historical range when available, but force it
                    # to include the exact stitched value shown on the chart.
                    try:
                        _lo = out.get("previous_day_prediction_low_f")
                        _hi = out.get("previous_day_prediction_high_f")
                        if _lo is None or pd.isna(_lo):
                            _lo = _latest
                        if _hi is None or pd.isna(_hi):
                            _hi = _latest
                        out["previous_day_prediction_low_f"] = min(
                            float(_lo), _latest
                        )
                        out["previous_day_prediction_high_f"] = max(
                            float(_hi), _latest
                        )
                    except Exception:
                        out["previous_day_prediction_low_f"] = _latest
                        out["previous_day_prediction_high_f"] = _latest
...
def previous_day_high_from_stitched_trajectory(
    snapshot_df,
    tz_name,
    target_date,
):
    """
    Derive the previous day's NWS predicted high from the exact stitched
    trajectory used by the Observed vs NWS prediction figure.

    For each forecast-valid hour, the figure uses the most recently stored NWS
    prediction available for that hour. The previous-day predicted high is then
    the maximum of those stitched hourly predictions on the previous local day.

    This is intentionally the display source of truth so the probability chart
    cannot say history is missing while the trajectory chart visibly contains it.
    """
    if snapshot_df is None or snapshot_df.empty:
        return None
    ...
    # Match latest_projection_chart exactly: forecasts captured after their
    # valid hour are excluded, then the latest stored forecast for each valid
    # hour is selected.
    ...
    prev_rows = work[work["time"].dt.date == prev_day].copy()
    if prev_rows.empty:
        return None

    prev_rows = prev_rows.sort_values(["valid_at", "snapshot_at"])
    stitched = prev_rows.groupby("valid_at", as_index=False).tail(1).copy()
    stitched = stitched.sort_values("valid_at")
    if stitched.empty:
        return None

    predicted_high = float(stitched["temp_f"].max())
    peak_rows = stitched[
        stitched["temp_f"] >= predicted_high - 0.05
    ].sort_values("valid_at")
    peak_row = peak_rows.iloc[0] if not peak_rows.empty else stitched.iloc[-1]

    # Snapshot that supplied the high point. Useful for traceability.
    source_snapshot_at = pd.Timestamp(peak_row["snapshot_at"])
    source_valid_at = pd.Timestamp(peak_row["valid_at"])
    source_snapshot_key = source_snapshot_at.floor("min")

    return {
        "latest_f": predicted_high,
        "peak_valid_at": source_valid_at,
        "source_snapshot_at": source_snapshot_at,
        "snapshot_key": source_snapshot_key,
        "source_label": "stitched Observed-vs-NWS trajectory",
    }
...
def latest_projection_chart(snapshot_df, observed_df, previous_observed_df, tz_name, target_date):
    ...
    # Previous-day NWS predicted high: take it directly from the same stitched
    # trajectory drawn in this figure.
    try:
        stitched_prev = previous_day_high_from_stitched_trajectory(
            snapshot_df,
            tz_name,
            target_date,
        )
        if stitched_prev is not None:
            pred_val = float(stitched_prev["latest_f"])
            label_time = pd.Timestamp(
                stitched_prev["peak_valid_at"]
            ).tz_convert(tz)
...
def max_projection_history_chart(
    ...
    # Adapt the horizontal scale to the actual snapshot window.
    span_hours = max(
        1.0,
        (
    ...
    # Keep the y scale tight enough to make 1°F revisions obvious.
    ...
    # Use deliberately conservative Altair/Vega-Lite schema properties here.
    # This avoids SchemaValidationError differences between local and
    # Streamlit Cloud Altair minor versions.
    base = (
        alt.Chart(history)
        .mark_line(
            point=True,
    ...
    # Highlight exact forecast revision moments.
    ...
    chart = (
        ...
        .properties(
            height=315,
            background="#0A0D18",
            title="How the predicted high has changed",
        )
        .configure_axis(
            labelFontSize=15,
            titleFontSize=16,
...
/* Kill V481's extra top-of-page modules even if stale markup survives. */
.we-mission-strip,
.we-weather-dock,
...
st.markdown(
    """
    <div class="we-hero we-hero-clean we-v481-hero">
      <div class="we-v481-copy">
        <div class="we-brand">WEATHEREDGE <span>ORBITAL</span></div>
        <div class="we-v481-edge">PROBABILITY FROM THE EDGE OF SPACE</div>
      </div>
      <div class="we-v481-satellite">🛰️</div>
      <div class="we-v481-sunset"></div>
      <div class="we-v481-palm">🌴</div>
      <div class="we-v481-weather">
...
.we-v481-hero:after {
  ...
  z-index:5 !important;
  background:linear-gradient(90deg,var(--v481-pink),var(--v481-violet),var(--v481-cyan)) !important;
  box-shadow:0 0 12px rgba(255,82,189,.34),0 0 10px rgba(66,220,255,.22) !important;
}
.we-v481-hero > * {position:relative;z-index:3 !important;}
.we-v481-copy {max-width:68%;}
.we-v481-hero .we-brand {
...
.we-v481-hero .we-brand span {
  margin-left:.38rem !important;
  font-size:.64rem !important;
  letter-spacing:.14em !important;
  color:var(--v481-pink) !important;
}
.we-v481-edge {
  margin-top:.55rem;
...
.quality-value {color:var(--v481-pink) !important;}
.quality-grade {color:var(--v481-cyan) !important;}
...
h2:after {
  ...
  margin-top:.28rem;
  border-radius:99px;
  background:linear-gradient(90deg,var(--v481-pink),var(--v481-cyan));
  opacity:.65;
}
...
# V481 visual refinement: slightly larger premium masthead, with all accidental
...
/* Explicitly remove old V468/V481 decorative text-bearing layers that can
...
.we-v481-sub { display:none !important; }
...
def conservative_probability_lower_bound(row):
    ...
    Sparse historical calibration is handled as a smooth uncertainty penalty,
    never as an automatic rejection.
    """
    ...
    n = max(0, int(safe_float(row.get("nws_sigma_samples"), 0)))

    uncertainty_penalty = min(0.18, 0.025 * max(0.0, sigma - 1.0))

    # Smoothly taper the sparse-history penalty:
    # 0 samples = 6 pp extra haircut, 4 samples = 3 pp, 8+ = 0 pp.
    sample_penalty = 0.06 * max(0.0, 8.0 - min(8.0, float(n))) / 8.0
...
if not ranking_df.empty:
    ranking_df["safety_prob_lower"] = ranking_df.apply(
        conservative_probability_lower_bound, axis=1
    )
    ranking_df["safety_edge_lower"] = (
        ranking_df["safety_prob_lower"] - ranking_df["ask"]
    )
    ranking_df["model_disagreement_f"] = ranking_df.apply(
        model_disagreement_f, axis=1
    )
...
model_agreement_mask = ranking_df["model_disagreement_f"].fillna(0.0) <= 4.0
...
def contract_status(row):
    ...
    if int(row.get("nws_sigma_samples", 0) or 0) < 8:
        reasons.append("limited calibration widens uncertainty")
    if float(row.get("safety_edge_lower", -1.0) or -1.0) < max(min_gap, 0.03):
...
if main_view == "Best Bets":
        ...
        st.markdown(
            ...
            <div class="filter-readout">
              ...
              <div class="filter-readout-grid">
                ...
                <span>Calibration <b>sparse history widens uncertainty</b></span>
    ...
    if not available_cities:
        ...
        forecast_url = (
            human_nws_forecast_url(city)
            or representative.get("nws_forecast_url")
        )
        observed_url = (
            human_observed_temperatures_url(city)
            or representative.get("observed_data_url")
        )
        ...
        if (
            ...
            st.warning(
                "Previous-day observed high is available, but the stored NWS snapshot "
                "history is not sufficient to reconstruct yesterday's trajectory. "
                "No previous-day predicted high is being inferred."
            )
