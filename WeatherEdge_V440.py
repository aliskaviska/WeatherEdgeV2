    ...
    if marker_rows:
        ...
        def marker_color(label):
            ...
            if label.startswith("Kalshi implied temp"):
                return "#FFD400"
            if label.startswith("Previous day predicted high"):
                return "#36C2FF"
...
def render_probability_chart_legend(row):
    ...
    if present(prev_pred_latest):
        items.append((
            f"Previous day predicted high · {float(prev_pred_latest):.1f}°F",
            "#63D8FF",
            "dot",
        ))
...
with st.sidebar:
    ...
    st.caption(
        ...
        "Settlement-source rounding safety is automatic: contracts touching either "
        "whole degree bracketing the observed high are excluded from Top Bets."
    )
