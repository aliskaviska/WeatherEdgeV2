...
def latest_projection_chart(snapshot_df, observed_df, previous_observed_df, tz_name, target_date):
    ...
    forecast = (
        ...
        .encode(
            ...
            color=alt.Color("Series:N", title=None, scale=alt.Scale(domain=["Previous day observed", "Observed", "Latest projection"], range=["#7899A6", "#7DE7F2", "#F4A7D3"])),
            tooltip=[
        ...
        previous_observed = (
            ...
            .encode(
                ...
                color=alt.Color(
                    "Series:N", title=None,
                    scale=alt.Scale(
                        domain=["Previous day observed", "Observed", "Latest projection"],
                        range=["#7899A6", "#7DE7F2", "#F4A7D3"],
                    ),
                ),
                tooltip=[
    ...
    chart = alt.layer(*layers).resolve_scale(color="shared").properties(
        ...
        title=alt.TitleParams(
            ...
            subtitle="Previous evening + current observations · dashed = newest stored forecast",
            anchor="start",
        ),
    ).configure_axis(
        labelFontSize=14,
...
if not qualified.empty:
    ...
    if observed_url:
        st.link_button("View official observed data ↗", observed_url, use_container_width=True)
    if r["suspicious"]:
