...
def build_nws_error_calibration(city, station_id, tz_name, cache_date=None):
    ...
    calibration_cache_path = cache_dir / f"calibration_{safe_city}_{cache_key_date}.pkl"
...
def build_city_rows(city, cfg):
    ...
    # GFS is display-only and is intentionally NOT fetched during the live
    # universe scan. City Explorer lazy-loads it only for the city/date opened.
    ens = None
    ...
    for m in markets:
        ...
        # fast scan. Previous-day history is display-only and lazy-loads later.
        observed_high = None
...
_PERSISTENT_SCAN_PATH = Path("/tmp/weatheredge_last_good_scan.pkl")
...
def previous_day_forecast_summary(city, tz_name, target_date, calibration=None):
    ...
    return {
        ...
        "source_label": (
            "stored NWS snapshots through previous-day peak"
            if peak_minutes is not None
            else "all stored NWS snapshots for previous day"
        ),
    }
...
def hydrate_city_explorer_row(row):
    """Merge lazy chart data into a copy of a selected market row."""
    try:
        out = row.copy()
