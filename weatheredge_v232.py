...
st.markdown("""
<div class="we-topbar">
  <div class="we-badge">⚡ WEATHEREDGE v2.3.2 • OBSERVED FLOOR</div>
...
for rank, (_, r) in enumerate(qualified.iterrows(), start=1):
    with st.container(border=True):
        ...
        obs_txt = "—" if obs is None or pd.isna(obs) else f"{int(obs)}°F"
        ...
        st.markdown(
            ...
            <div class="we-metricgrid">
              ...
              <div class="we-metric">
                <div class="we-metric-icon">▮▮</div>
                <div class="we-metric-label">Model Median</div>
                <div class="we-metric-value">{med_txt}</div>
              </div>
              <div class="we-metric">
                <div class="we-metric-icon">🏷️</div>
        ...
        observed = r.get("observed_high_f")
        remaining_txt = "—" if remaining is None or pd.isna(remaining) else f"{remaining:.0f}°F"
        observed_txt = "—" if observed is None or pd.isna(observed) else f"{observed:.0f}°F"
        st.caption(
