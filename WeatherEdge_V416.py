...
def forecast_range_summary_chart(row, show_bet_overlay=False):
    ...
    # The overlay is a full-height bucket band, colored by recommended side.
    ...
    if show_bet_overlay and bet_side in ("YES", "NO"):
        ...
        if bet_kind == "range" and bet_low is not None and bet_high is not None:
            # Kalshi integer-temperature buckets use half-degree boundaries.
            band_lo, band_hi = bet_low - 0.5, bet_high + 0.5
        elif bet_kind == "above" and bet_low is not None:
            # For threshold contracts, show a compact 1°F reference band.
        ...
        if band_lo is not None and band_hi is not None:
            ...
            top_color = "#16D47B" if bet_side == "YES" else "#FF4050"
            ...
            # With no y/y2 encoding, the rectangle spans the full plot height.
            layers.append(
                alt.Chart(bet_df)
                .mark_rect(
                    color=top_color,
...
def render_probability_chart_legend(row):
    ...
    if status_kind == "best":
        bet_side = str(row.get("side") or "").upper()
        top_color = "#16D47B" if bet_side == "YES" else "#FF4050"
        items.append((f"{status_text} · {bet_side}", top_color, "band"))
