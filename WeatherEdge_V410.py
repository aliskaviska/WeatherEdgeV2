...
def forecast_range_summary_chart(row, show_bet_overlay=False):
    ...
    if show_bet_overlay and bet_side in ("YES", "NO"):
        ...
        if band_lo is not None and band_hi is not None:
            bet_df = pd.DataFrame([{
                ...
                "x2": band_hi,
                "y1": 0.0,
                "y2": max(0.45, float(dist_df["density"].max()) * 1.12 if not dist_df.empty else 0.45),
                "label": f"TOP BET · {bet_side}",
            }])
            layers.append(
                alt.Chart(bet_df).mark_rect(color="#FFD166", opacity=0.11).encode(
                    x=alt.X("x1:Q", scale=alt.Scale(domain=[x_min, x_max])),
                    x2="x2:Q",
                    y=alt.Y("y1:Q"),
                    y2="y2:Q",
                    tooltip=[alt.Tooltip("label:N", title="Recommended contract")],
                )
            )
    ...
    if previous_3day_avg is not None:
        marker_specs.append(("Previous 3-day avg high", previous_3day_avg, ""))
    ...
    if marker_rows:
        ...
        palette = [
            "#D9CFFF", "#B79CFF", "#FF8FCB",
            "#FFD166", "#7CE7B2", "#F6B26B",
        ][:len(marker_rows)]
    ...
    subtitle = (
        "NWS distribution is the calibrated probability model used by WeatherEdge. "
        "GFS distribution is ensemble reference only. Previous-day and 3-day observed highs appear when complete data is available."
    )
    ...
    # A dedicated in-figure legend is more reliable and readable on mobile than
    # stacking several independent Vega-Lite color legends at the top of the plot.
    legend_items = [
        ("NWS probability", "#B79CFF"),
        ("GFS ensemble", "#73E5F2"),
    ]
    if nws_raw is not None:
        legend_items.append(("Raw NWS high", "#D9CFFF"))
    if nws_center is not None:
        legend_items.append(("Calibrated NWS center", "#B79CFF"))
    if observed is not None:
        legend_items.append(("Observed so far", "#FF8FCB"))
    if implied is not None:
        legend_items.append(("Kalshi implied temp", "#FFD166"))
    if previous_day_high is not None:
        legend_items.append(("Previous day high", "#7CE7B2"))
    if previous_3day_avg is not None:
        legend_items.append(("Previous 3-day avg high", "#F6B26B"))
    ...
    legend_height = max(56, 28 * ((len(legend_items) + 1) // 2))

    legend_points = (
        alt.Chart(legend_df)
        .mark_point(filled=True, size=120)
        .encode(
            x=alt.X("x:Q", axis=None, scale=alt.Scale(domain=[-0.08, 1.55])),
            y=alt.Y("y:Q", axis=None, sort="ascending"),
            color=alt.Color("color:N", scale=None, legend=None),
            tooltip=[alt.Tooltip("label:N", title="Legend")],
        )
    )
    legend_text = (
        alt.Chart(legend_df)
    ...
    return (
        alt.vconcat(main_chart, legend_chart, spacing=6)
        .configure_axis(
            labelFontSize=13,
            titleFontSize=15,
            labelColor="#F1EDF7",
...
st.caption("Best Bets, watchlist tracking, and a city-by-city weather market explorer.")
...
main_view = st.radio(
    "Main section",
    ["Best Bets", "My Watchlist", "City Explorer"],
    horizontal=True,
    key="main_view",
...
if main_view == "Best Bets":
    ...
    if qualified.empty:
        ...
        st.button(
            "Explore this bet in City Explorer →",
            use_container_width=True,
            on_click=go_to_city_explorer,
            args=(r["city"], f"{r['market_ticker']}|{r['side']}"),
        )
    ...
    if not watched_keys:
        st.info("Your watchlist is empty. Add contracts from Best Bets or City Explorer.")
    else:
        ...
        if live_keys:
            ...
            render_contract_detail(r, show_weather=False)
            st.button(
                "Explore this watchlist bet in City Explorer →",
                use_container_width=True,
                on_click=go_to_city_explorer,
                args=(r["city"], f"{r['market_ticker']}|{r['side']}"),
            )
