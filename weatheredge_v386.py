...
def forecast_range_summary_chart(row):
    ...
    # Highlight the exact Kalshi contract interval for the recommended side.
    # For open-ended contracts, extend the band to the visible chart boundary.
    ...
    if band_lo is not None and band_hi is not None and bet_side in ("YES", "NO"):
        bet_df = pd.DataFrame([{
            "low": band_lo, "high": band_hi, "lane": "Daily high",
            "label_x": (band_lo + band_hi) / 2, "label": f"BET {bet_side}",
        }])
        layers.append(
            alt.Chart(bet_df)
            .mark_rect(opacity=0.16, color="#FFD166")
            .encode(
                ...
                tooltip=[
                    ...
                    alt.Tooltip("low:Q", title="Contract range starts", format=".0f"),
                    alt.Tooltip("high:Q", title="Contract range ends", format=".0f"),
                ],
            )
        )
        layers.append(
            alt.Chart(bet_df)
            .mark_text(dy=35, fontSize=13, fontWeight="bold", color="#FFD166")
            .encode(
                x=alt.X("label_x:Q", scale=alt.Scale(domain=[domain_min, domain_max])),
                y=alt.Y("lane:N", axis=None),
                text="label:N",
            )
        )
    ...
    return (
        ...
        .properties(
            ...
            title=alt.TitleParams(
                ...
                subtitle="GFS range + median, latest NWS high, observed high, and recommended Kalshi interval",
                anchor="start",
            ),
        )
        .configure_axis(
