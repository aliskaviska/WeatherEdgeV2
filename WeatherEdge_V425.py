...
def forecast_range_summary_chart(row, show_bet_overlay=False):
    ...
    if marker_rows:
        ...
        # Direct labels make exact reference temperatures readable without
        # requiring the user to triangulate from the axis.
        marker_df["value_label"] = marker_df.apply(
            lambda r: f"{r['Marker'].split(' · ')[0]}  {r['temperature']:.1f}°",
            axis=1,
        )
        layers.append(
            alt.Chart(marker_df)
            ...
            .encode(
                ...
                y=alt.value(12),
                text="value_label:N",
                color=alt.Color("Marker:N", scale=marker_scale, legend=None),
            )
        )
...
def latest_projection_chart(snapshot_df, observed_df, previous_observed_df, tz_name, target_date):
    ...
    common_x = alt.X(
        ...
        axis=alt.Axis(format="%b %-d, %-I %p", labelAngle=-30, tickCount=12, grid=True, gridOpacity=0.38, tickSize=7),
        scale=alt.Scale(domain=[x_min, x_max]),
    )
    common_y = alt.Y(
        "temp_f:Q", title="Temperature (°F)", scale=alt.Scale(zero=False),
        axis=alt.Axis(tickCount=11, grid=True, gridOpacity=0.40, tickSize=7, format=".0f"),
    )
    ...
    if not available_cities:
        ...
        with bet_panel:
            ...
            st.markdown(
                ...
                f"NWS chance {selected_row['conservative_prob']*100:.0f}% · "
        ...
        m1, m2, m3 = st.columns(3)
        adjusted_center = (
            float(representative["nws_high_f"])
            + float(representative.get("nws_bias_f") or 0.0)
            + float(representative.get("trajectory_adjustment_f") or 0.0)
        )
        m1.metric(
            "Live-adjusted NWS center",
            f"{adjusted_center:.1f}°F",
            delta=(
                None
                if abs(float(representative.get("trajectory_adjustment_f") or 0.0)) < 0.05
                else f"{float(representative.get('trajectory_adjustment_f') or 0.0):+.1f}° live trajectory"
            ),
        )
