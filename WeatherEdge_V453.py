...
with st.sidebar:
    ...
    st.caption(
        ...
        "excluded; WeatherEdge will not recommend any side Kalshi prices at 5% implied "
        "probability or less; Top Bets also require enough historical calibration, a "
        "positive edge after an uncertainty haircut, no strong near-peak live conflict, "
        "and no major unresolved model disagreement."
    )
...
def conservative_probability_lower_bound(row):
    ...
    """
    p = float(row.get("conservative_prob", 0.5))
    sigma = float(row.get("nws_sigma_f", 2.0) or 2.0)
    n = int(row.get("nws_sigma_samples", 0) or 0)
...
historical_depth_mask = df["nws_sigma_samples"].fillna(0).astype(int) >= 8

trajectory_conflict_mask = ~(
    df["trajectory_current_gap_f"].notna()
    & (df["hours_to_expected_peak"].fillna(99) <= 3)
    & (df["trajectory_current_gap_f"].abs() >= 1.5)
)
