    ...
    for p in forecast["properties"]["periods"]:
        if p.get("isDaytime"):
            rows.append({
                ...
                "nws_forecast_url": f"https://forecast.weather.gov/MapClick.php?lat={lat}&lon={lon}",
            })
    return rows
...
st.set_page_config(page_title="WeatherEdge Dark", page_icon="🌦️", layout="centered")
...
.stApp {
    background:
      radial-gradient(circle at 15% 0%, rgba(31,255,117,.08), transparent 28rem),
      linear-gradient(180deg, #050a10 0%, #071019 55%, #04080d 100%);
    color: #f3f7fb;
}
.block-container {
    max-width: 760px;
...
h1, h2, h3, p, label, .stMarkdown { color: #f3f7fb; }
[data-testid="stSidebar"] { background: #08111a; }
...
[data-testid="stMetricLabel"] { color: #c6d0dc; }
...
a[data-testid="stLinkButton"] {
    border: 1px solid #35e978 !important;
    color: #55f58c !important;
    background: rgba(20,110,55,.10) !important;
}
.weatheredge-badge {
...
.hero-sub { color:#9daaba; margin-bottom:1.5rem; }
.card-title { font-size:1.45rem; font-weight:850; margin-bottom:.25rem; }
.card-sub { font-size:1.05rem; color:#aab5c2; margin-bottom:.7rem; }
.small-note { font-size:.9rem; color:#8f9baa; }
hr { border-color:#243343; }
</style>
""", unsafe_allow_html=True)

st.markdown('<div class="weatheredge-badge">WEATHEREDGE v2.2 • LIVE NWS HIGH • DARK BUILD</div>', unsafe_allow_html=True)
st.markdown('<div class="hero-title">🌦️ WeatherEdge</div>', unsafe_allow_html=True)
st.markdown('<div class="hero-sub">Forecast-aligned Kalshi weather candidates. Official NWS daily highs, live market prices.</div>', unsafe_allow_html=True)
...
for rank, (_, r) in enumerate(qualified.iterrows(), start=1):
    with st.container(border=True):
        ...
        if r.get("nws_forecast_url"):
            st.link_button(
                "🌤️ Open NWS forecast for this location ↗",
                r["nws_forecast_url"],
