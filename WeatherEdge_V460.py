...
def build_city_rows(city, cfg):
    ...
    # Series/event metadata is display-only and can cost many serialized Kalshi
    # API calls. Skip it in the ranking scan and lazy-load when a bet is opened.
    series_info = {}
    ...
    for m in markets:
        ...
        if d not in observed_context_cache:
            try:
                observed_context_cache[d] = observed_daily_high_details(
                    cfg.get("station_id"), cfg["tz"], d
                )
            except Exception:
                observed_context_cache[d] = (None, None)
        observed_high, observed_high_time = observed_context_cache[d]
