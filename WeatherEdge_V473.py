...
/* Kill V473's extra top-of-page modules even if stale markup survives. */
.we-mission-strip,
.we-weather-dock,
...
st.markdown(
    """
    <div class="we-hero we-hero-clean we-v473-hero">
      <div class="we-v473-copy">
        <div class="we-brand">WEATHEREDGE <span>ORBITAL</span></div>
        <div class="we-v473-edge">PROBABILITY FROM THE EDGE OF SPACE</div>
      </div>
      <div class="we-v473-satellite">🛰️</div>
      <div class="we-v473-sunset"></div>
      <div class="we-v473-palm">🌴</div>
      <div class="we-v473-weather">
...
.we-v473-hero:after {
  ...
  z-index:5 !important;
  background:linear-gradient(90deg,var(--v473-pink),var(--v473-violet),var(--v473-cyan)) !important;
  box-shadow:0 0 12px rgba(255,82,189,.34),0 0 10px rgba(66,220,255,.22) !important;
}
.we-v473-hero > * {position:relative;z-index:3 !important;}
.we-v473-copy {max-width:68%;}
.we-v473-hero .we-brand {
...
.we-v473-hero .we-brand span {
  margin-left:.38rem !important;
  font-size:.64rem !important;
  letter-spacing:.14em !important;
  color:var(--v473-pink) !important;
}
.we-v473-edge {
  margin-top:.55rem;
...
.quality-value {color:var(--v473-pink) !important;}
.quality-grade {color:var(--v473-cyan) !important;}
...
h2:after {
  ...
  margin-top:.28rem;
  border-radius:99px;
  background:linear-gradient(90deg,var(--v473-pink),var(--v473-cyan));
  opacity:.65;
}
...
# V473 visual refinement: slightly larger premium masthead, with all accidental
...
/* Explicitly remove old V468/V473 decorative text-bearing layers that can
...
.we-v473-sub { display:none !important; }
...
ranking_df["safety_prob_lower"] = ranking_df.apply(conservative_probability_lower_bound, axis=1)
ranking_df["safety_edge_lower"] = ranking_df["safety_prob_lower"] - ranking_df["ask"]
ranking_df["model_disagreement_f"] = ranking_df.apply(model_disagreement_f, axis=1)
