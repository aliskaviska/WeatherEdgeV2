...
def forecast_range_summary_chart(row):
    """Daily-high summary with forecast, observations, Kalshi temperature, and ranked bet interval."""
    ...
    top_rank = row.get("top_bet_rank")
    try:
        top_rank = int(top_rank) if pd.notna(top_rank) else None
    except (TypeError, ValueError):
        top_rank = None

    values = [v for v in (low, high, median, projected, observed, implied, bet_low, bet_high) if v is not None]
    if not values:
        return None
    ...
    colors = {
        ...
        "Kalshi implied temp": "#F7C873",
    }

    # Show the selected/recommended contract as a full-size chart series, with its
    # rank and side in the legend rather than tiny text hanging off the plot.
    bet_label = None
    if bet_side in ("YES", "NO"):
        bet_label = f"Top Bet #{top_rank} · {bet_side}" if top_rank else f"Selected bet · {bet_side}"
        colors[bet_label] = "#9FF0B6" if bet_side == "YES" else "#FF9F9F"
    ...
    if low is not None and high is not None:
        ...
        layers.append(alt.Chart(d).mark_rule(strokeWidth=15, opacity=.78, strokeCap="round").encode(
            x=x_axis, x2="high:Q", y=alt.Y("lane:N", axis=None),
            color=alt.Color("Series:N", scale=alt.Scale(domain=domain, range=rng), legend=alt.Legend(title=None)),
            tooltip=[alt.Tooltip("low:Q", title="GFS low", format=".1f"),
                     alt.Tooltip("high:Q", title="GFS high", format=".1f")],
        ))

    # Contract threshold/bracket is drawn with the same weight as the GFS range.
    band_lo = band_hi = None
    if bet_kind == "range" and bet_low is not None and bet_high is not None:
        band_lo, band_hi = bet_low, bet_high
    elif bet_kind == "above" and bet_low is not None:
        band_lo, band_hi = bet_low, domain_max
    elif bet_kind in ("below", "below_equal") and bet_high is not None:
        band_lo, band_hi = domain_min, bet_high

    if bet_label and band_lo is not None and band_hi is not None:
        bd = pd.DataFrame([{"low": band_lo, "high": band_hi, "lane": "Daily high", "Series": bet_label}])
        layers.append(alt.Chart(bd).mark_rule(strokeWidth=15, opacity=.80, strokeCap="round").encode(
            x=x_axis, x2="high:Q", y=alt.Y("lane:N", axis=None),
            color=alt.Color("Series:N", scale=alt.Scale(domain=domain, range=rng), legend=alt.Legend(title=None)),
            tooltip=[alt.Tooltip("Series:N", title="Bet"),
                     alt.Tooltip("low:Q", title="Contract low", format=".1f"),
                     alt.Tooltip("high:Q", title="Contract high", format=".1f")],
        ))

    pts = []
    for label, value in (
        ("GFS median", median),
        ("NWS projected high", projected),
        ("Observed high", observed),
        ("Kalshi implied temp", implied),
    ):
        if value is not None:
            pts.append({"temperature": value, "lane": "Daily high", "Series": label})
    if pts:
        d = pd.DataFrame(pts)
        layers.append(alt.Chart(d).mark_point(filled=True, size=230, stroke="#11121B", strokeWidth=2).encode(
            ...
            color=alt.Color("Series:N", scale=alt.Scale(domain=domain, range=rng), legend=alt.Legend(title=None)),
            tooltip=[alt.Tooltip("Series:N", title="Measure"),
        ...
        layers.append(alt.Chart(d).mark_text(dy=-22, fontSize=15, fontWeight="bold").encode(
            ...
            color=alt.Color("Series:N", scale=alt.Scale(domain=domain, range=rng), legend=None),
        ))

    return alt.layer(*layers).resolve_scale(color="shared").properties(
        height=205, background="#11121B",
        title=alt.TitleParams(
            text="Forecast range & daily highs",
            subtitle="GFS range/median, NWS high, observed high, Kalshi implied temperature, and selected bet",
            anchor="start",
        ),
    ).configure_axis(
        labelFontSize=14, titleFontSize=16, labelColor="#F1EDF7", titleColor="#FAF7FF",
        gridColor="#777185", gridOpacity=.28, domainColor="#AAA3B5", tickColor="#AAA3B5", tickWidth=1.5,
    ).configure_legend(
        orient="top", direction="horizontal", columns=2, title=None, labelFontSize=13,
        labelColor="#F1EDF7", symbolSize=220, padding=8,
    ).configure_title(
...
def latest_projection_chart(snapshot_df, observed_df, previous_observed_df, tz_name, target_date):
    ...
    if x_min <= now_local <= x_max:
        ...
        now_label = f"NOW · {now_local:%b %-d, %Y · %-I:%M %p %Z}"
        layers.append(alt.Chart(now_df).mark_text(
            text=now_label, color="#8CEAF2", fontSize=13, fontWeight="bold", angle=0,
            align="left", baseline="top", dx=6, dy=5
...
st.caption("Top Bets and a city-by-city weather market explorer.")

st.divider()

# Keep the main navigation unmistakably interactive and simple: one ranked-bet
# destination plus the city-level explorer.
if "main_view" not in st.session_state or st.session_state.main_view not in ("Top Bets", "City Explorer"):
    st.session_state.main_view = "Top Bets"
...
main_view = st.radio(
    "Navigate",
    ["Top Bets", "City Explorer"],
    horizontal=True,
    key="main_view",
    help="Choose Top Bets for ranked recommendations or City Explorer for all configured cities.",
)

with st.expander("Top Bets settings", expanded=(main_view == "Top Bets")):
    top_n = st.slider("Number of Top Bets", 1, 12, 5, 1)
    min_nws_chance = st.slider(
        ...
        value=70,
        step=1,
        help="Bets below this NWS-based win probability are excluded before ranking.",
    ) / 100
    min_gap = st.slider(
        "Minimum Weather Edge", 0, 30, 5, 1,
        help="Minimum percentage-point advantage between the NWS-based probability and the market price.",
    ) / 100

with st.sidebar:
    st.markdown("### Market scan")
    scan_mode = st.radio("Cities to scan", ["All cities", "One city"], index=0)
...
# Mark ranked Top Bets back onto the master dataframe so City Explorer and
# every chart can display the same rank/YES-NO identity consistently.
df["top_bet_rank"] = pd.NA
for rank, (_, ranked_row) in enumerate(qualified.iterrows(), start=1):
    mask = (df["market_ticker"] == ranked_row["market_ticker"]) & (df["side"] == ranked_row["side"])
    df.loc[mask, "top_bet_rank"] = rank
qualified = df[df["top_bet_rank"].notna()].copy().sort_values("top_bet_rank")


def contract_status(row):
    """Explain whether a contract is currently ranked as a Top Bet."""
    rank = row.get("top_bet_rank")
    if pd.notna(rank):
        return f"TOP BET #{int(rank)}", "best"
    ...
    return "NOT CURRENTLY A TOP BET" + (f" · {' · '.join(reasons)}" if reasons else ""), "other"


def render_contract_detail(r, show_weather=True):
    """One consistent contract detail view used by Top Bets and City Explorer."""
...
# Main content. Top Bets now exposes the full weather evidence directly, while
# City Explorer remains the common destination for browsing any city/contract.
if main_view == "Top Bets":
    st.subheader("Top Bets")
    st.caption("Ranked contracts that pass your active NWS-chance and Weather Edge filters. Select one to see its weather evidence immediately.")

    if qualified.empty:
        st.info(
            f"No contracts currently pass the filters: "
            f"NWS-based chance ≥ {min_nws_chance*100:.0f}% and Weather Edge ≥ {min_gap*100:.0f} pp."
        )
    else:
        top_rows = [r for _, r in qualified.iterrows()]
        top_labels = [
            f"#{int(r['top_bet_rank'])} · {r['city']} · {r['side']} · {r['bet_quality_score']:.0f}/100"
            for r in top_rows
        ]
        selected = st.radio(
            "Choose a Top Bet",
        ...
        r = top_rows[top_labels.index(selected)]

        # Full figures live here now. No more detour just to inspect the weather basis.
        render_contract_detail(r, show_weather=True)

        if st.button("Open this bet in City Explorer →", use_container_width=True):
            st.session_state.explorer_city = r["city"]
            st.session_state.explorer_contract = f"{r['market_ticker']}|{r['side']}"
    ...
    st.caption(
        "Browse every configured Kalshi temperature city whether or not it currently has a Top Bet."
    )
    ...
    if not available_cities:
        ...
        # City-level weather figures use the strongest contract only as a context row.
        range_chart = forecast_range_summary_chart(representative)
        if range_chart is not None:
            st.markdown("<div class='section-kicker'>FORECAST RANGE</div>", unsafe_allow_html=True)
            st.altair_chart(range_chart, use_container_width=True)
        ...
        st.caption(
            ...
            "Current Top Bets are marked with a star and rank."
        )
        ...
        for row in contract_rows:
            ...
            badge = f"★ #{int(row['top_bet_rank'])} · " if kind == "best" else ""
