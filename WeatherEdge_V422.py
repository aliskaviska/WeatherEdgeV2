...
def scan_live_market_universe():
    """
    Build the full WeatherEdge market dataframe once, then reuse it across
    ordinary Streamlit reruns for 5 minutes.

    This is intentionally cached ABOVE the page/navigation layer so changing
    tabs, opening a bet, toggling watch status, or adjusting non-data UI does
    not trigger a fresh all-city Kalshi/NWS/GFS scan.
    """
    rows = []
    errors = []
    ...
    for city, cfg in PRESETS.items():
        try:
            rows.extend(build_city_rows(city, cfg))
...
with st.sidebar:
    st.caption("Live market scan is cached for 5 minutes. Use “Refresh live data” on the main page when you want an immediate update.")
...
with st.spinner("Refreshing live markets and weather data…"):
