    ...
    for m in markets:
        ...
        for side, ask, p in side_data:
            ...
            rows.append({
                ...
                "nws_public_url": f"https://forecast.weather.gov/MapClick.php?lat={cfg['lat']}&lon={cfg['lon']}",
                "date": d,
...
st.markdown("""
<div class="we-topbar">
  <div class="we-badge">⚡ WEATHEREDGE v2.1.0</div>
...
<div class="we-section-sub">Sorted by model/market gap (high to low)</div>
""", unsafe_allow_html=True)
...
for rank, (_, r) in enumerate(qualified.iterrows(), start=1):
    with st.container(border=True):
        ...
        st.markdown(
            f"""
            <div class="we-nwsbox">
              <a href="{nws_url}" target="_blank">🌤️ Open NWS forecast for this location ↗</a>
            </div>
            <div class="we-url">{nws_url}</div>
            """,
            unsafe_allow_html=True,
        )
        ...
        with c1:
            st.metric("🌡️ NWS Forecast", "—" if pd.isna(r["nws_high_f"]) else f"{int(r['nws_high_f'])}°F")
        with c2:
            obs = r["observed_high_f"] if "observed_high_f" in r.index else None
            st.metric("↗ Observed High", "—" if obs is None or pd.isna(obs) else f"{int(obs)}°F")
        with c3:
            st.metric("▮▮ Model Median", "—" if pd.isna(r["ensemble_median_f"]) else f"{r['ensemble_median_f']:.1f}°F")
        with c4:
        ...
        st.markdown(
            ...
            <div class="we-probgrid">
              <div class="we-probbox">
                <div class="we-probtitle">Conservative Chance</div>
                <div class="we-probvalue">{fmt_pct(r['conservative_prob'])}</div>
                <div class="we-agree">Forecasts agree on this side</div>
              </div>
              <div class="we-probbox">
                <div class="we-probtitle">Model/Market Gap</div>
                <div class="we-gapvalue">{r['conservative_edge']*100:+.1f} pp</div>
                <div class="we-warning">Large gap. Verify contract.</div>
              </div>
            </div>
            """,
            unsafe_allow_html=True,
