...
def latest_projection_chart(snapshot_df, observed_df, previous_observed_df, tz_name, target_date):
    ...
    # Build ONE NWS prediction line across the target day. For every valid time,
    # retain the most recently collected forecast that contained that time.
    target_rows = work[work["time"].dt.date == target_date].copy()
    if target_rows.empty:
        return None, latest_key
    target_rows = target_rows.sort_values(["valid_at", "snapshot_at"])
    forecast = target_rows.groupby("valid_at", as_index=False).tail(1).copy()
...
.signal-value {font-size:1.28rem; line-height:1.1; color:#FCFAFF; font-weight:780;}
.signal-sub {font-size:.78rem; line-height:1.3; color:#BDB6C8; margin-top:.22rem;}
@media (max-width:520px) {.signal-strip {gap:.5rem;} .signal-tile {padding:.7rem .72rem;} .signal-value {font-size:1.16rem;} .signal-label {font-size:.63rem;}}
