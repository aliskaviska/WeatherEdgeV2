...
def render_probability_chart_legend(row):
    """Readable HTML legend for the daily-high probability chart."""
    def present(value):
        try:
    ...
    items = [
        ...
        ("GFS ensemble probability", "#73E5F2"),
    ]

    if present(row.get("nws_high_f")):
        items.append(("Raw NWS high", "#D9CFFF"))
    if present(row.get("nws_high_f")):
        items.append(("Calibrated NWS center", "#B79CFF"))
    ...
    if present(row.get("observed_high_f")):
        if present(obs_time):
            try:
                ...
                pass
        items.append((obs_label, "#FF8FCB"))

    if present(row.get("kalshi_implied_temp_f")):
        items.append(("Kalshi implied temp", "#FFD166"))
    ...
    if present(row.get("previous_day_high_f")):
        if present(prev_time):
            try:
                ...
                pass
        items.append((prev_label, "#7CE7B2"))

    if present(row.get("previous_3day_avg_high_f")):
        items.append(("Previous 3-day avg high", "#F6B26B"))

    cells = "".join(
        f"<div style='display:flex;align-items:center;gap:.48rem;min-width:0;'>"
        f"<span style='width:12px;height:12px;border-radius:50%;background:{color};"
        f"display:inline-block;flex:0 0 12px;border:1px solid rgba(255,255,255,.35)'></span>"
        f"<span style='color:#F1EDF7;font-size:.86rem;line-height:1.25'>{label}</span>"
        f"</div>"
        for label, color in items
    )

    st.markdown(
        "<div style='margin:.35rem 0 1rem;padding:.75rem .85rem;border-radius:12px;"
        "background:#151620;border:1px solid rgba(255,255,255,.14)'>"
        "<div style='font-size:.72rem;font-weight:800;letter-spacing:.11em;"
        "color:#D8CFFF;margin-bottom:.55rem'>LEGEND</div>"
        f"<div style='display:grid;grid-template-columns:repeat(2,minmax(0,1fr));"
        f"gap:.55rem .8rem'>{cells}</div></div>",
