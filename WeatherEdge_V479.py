...
def latest_projection_chart(snapshot_df, observed_df, previous_observed_df, tz_name, target_date):
    ...
    # Previous-day NWS predicted high: use the exact same canonical value as
    # the probability chart and legend.
    try:
        canonical_prev = canonical_previous_day_nws_high_from_snapshots(
            snapshot_df,
            tz_name,
            target_date,
        ...
        if canonical_prev is not None:
            pred_val = float(canonical_prev["latest_f"])
            chosen_key = canonical_prev.get("snapshot_key")

            chosen_rows = snapshot_df[
                snapshot_df["snapshot_key"] == chosen_key
            ].copy()
            chosen_rows["valid_local"] = chosen_rows["valid_at"].dt.tz_convert(tz)
            chosen_rows = chosen_rows[
                chosen_rows["valid_local"].dt.date == prev_day
            ]
            peak_rows = chosen_rows[
                pd.to_numeric(
                    chosen_rows["temp_f"], errors="coerce"
                ) >= pred_val - 0.05
            ].sort_values("valid_local")

            label_time = (
                peak_rows.iloc[0]["valid_local"]
                if not peak_rows.empty
                else canonical_prev.get("latest_snapshot_local")
            )
...
/* Kill V479's extra top-of-page modules even if stale markup survives. */
.we-mission-strip,
.we-weather-dock,
...
st.markdown(
    """
    <div class="we-hero we-hero-clean we-v479-hero">
      <div class="we-v479-copy">
        <div class="we-brand">WEATHEREDGE <span>ORBITAL</span></div>
        <div class="we-v479-edge">PROBABILITY FROM THE EDGE OF SPACE</div>
      </div>
      <div class="we-v479-satellite">🛰️</div>
      <div class="we-v479-sunset"></div>
      <div class="we-v479-palm">🌴</div>
      <div class="we-v479-weather">
...
.we-v479-hero:after {
  ...
  z-index:5 !important;
  background:linear-gradient(90deg,var(--v479-pink),var(--v479-violet),var(--v479-cyan)) !important;
  box-shadow:0 0 12px rgba(255,82,189,.34),0 0 10px rgba(66,220,255,.22) !important;
}
.we-v479-hero > * {position:relative;z-index:3 !important;}
.we-v479-copy {max-width:68%;}
.we-v479-hero .we-brand {
...
.we-v479-hero .we-brand span {
  margin-left:.38rem !important;
  font-size:.64rem !important;
  letter-spacing:.14em !important;
  color:var(--v479-pink) !important;
}
.we-v479-edge {
  margin-top:.55rem;
...
.quality-value {color:var(--v479-pink) !important;}
.quality-grade {color:var(--v479-cyan) !important;}
...
h2:after {
  ...
  margin-top:.28rem;
  border-radius:99px;
  background:linear-gradient(90deg,var(--v479-pink),var(--v479-cyan));
  opacity:.65;
}
...
# V479 visual refinement: slightly larger premium masthead, with all accidental
...
/* Explicitly remove old V468/V479 decorative text-bearing layers that can
...
.we-v479-sub { display:none !important; }
    ...
    if not available_cities:
        ...
        if (
            ...
            st.warning(
                ...
                "reconstruct a valid stored NWS forecast that still covered that day's "
                "high-temperature window. No previous-day predicted high is being inferred."
            )
