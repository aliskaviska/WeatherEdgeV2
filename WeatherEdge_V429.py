...
def nws_sigma_f(hours_to_peak, calibration=None):
    """
    Use day-level empirical NWS forecast error for the current peak-relative lead bucket.

    Snapshot runs from the same day are highly correlated, so calibration counts
    independent completed DAYS rather than treating every stored snapshot as a
    separate forecast-error sample. Until at least 10 days are available, retain
    the conservative heuristic width.
    """
    bucket = _lead_bucket(hours_to_peak)
    fallback = heuristic_nws_sigma_f(hours_to_peak)
    if calibration and bucket in calibration:
        row = calibration[bucket]
        n_days = int(row.get("n_days", row.get("n", 0)) or 0)
        empirical = row.get("sigma_f")
        if n_days >= 10 and empirical is not None:
            # Never allow a short empirical history to become dramatically
            # narrower than the conservative lead-time fallback.
            return max(0.75 * fallback, 0.70, float(empirical))
    return fallback
...
def nws_yes_probability(
    ...
    try:
        sigma *= max(0.70, min(1.35, float(trajectory_sigma_multiplier)))
    except Exception:
        pass
...
def build_nws_error_calibration(city, station_id, tz_name):
    """
    Learn NWS daily-high error from independent completed days.

    Multiple stored snapshots from one day are NOT independent observations. For
    each completed day and peak-relative lead bucket, WeatherEdge first collapses
    that day's snapshots to one representative forecast, then measures the error
    against the final observed daily high. Distribution width is estimated across
    days, so a large miss on a day such as yesterday genuinely widens the model
    instead of being diluted by dozens of correlated snapshots.
    """
    ...
    # Keep a reasonably recent rolling history so the error model can adapt while
    # still containing enough independent weather days to estimate uncertainty.
    ...
    for _, row in projected.iterrows():
        ...
        snapshot_samples.append({
            "target_date": d,
            "bucket": _lead_bucket(hours_to_peak),
            "projected_high_f": float(row["projected_high_f"]),
            "actual_high_f": float(stat["high_f"]),
            "hours_to_peak": hours_to_peak,
        })

    calibration = {}
    if snapshot_samples:
        sdf = pd.DataFrame(snapshot_samples)

        # Collapse correlated intraday snapshots to ONE representative forecast
        # error per day, per comparable lead bucket.
        daily = (
            sdf.groupby(["target_date", "bucket"], as_index=False)
            .agg(
                projected_high_f=("projected_high_f", "median"),
                actual_high_f=("actual_high_f", "first"),
                hours_to_peak=("hours_to_peak", "median"),
                snapshots=("projected_high_f", "size"),
            )
        )
        daily["error_f"] = daily["actual_high_f"] - daily["projected_high_f"]

        for bucket, grp in daily.groupby("bucket"):
            errors = grp["error_f"].dropna().astype(float)
            n_days = int(len(errors))
            if n_days == 0:
                continue

            bias = float(errors.mean())
            residuals = errors - bias
            residual_sigma = float(residuals.std(ddof=1)) if n_days >= 2 else 0.0
            mae = float(errors.abs().mean())
            residual_abs = residuals.abs()
            residual_mae = float(residual_abs.mean())
            # A robust tail-aware residual width. Bias shifts the center; residual
            # dispersion controls width, avoiding double-counting a systematic bias.
            residual_q80 = float(residual_abs.quantile(0.80))
            empirical_sigma = max(
                residual_sigma,
                residual_mae * 1.15,
                residual_q80 / 1.2816,
            ...
            calibration[bucket] = {
                "n": n_days,          # legacy key used elsewhere
                "n_days": n_days,
                "sigma_f": empirical_sigma,
                "bias_f": bias,
                "mae_f": mae,
                "residual_q80_f": residual_q80,
                "snapshot_count": int(grp["snapshots"].sum()),
            }
...
def build_city_rows(city, cfg):
    ...
    for m in markets:
        ...
        for side, ask, p_nws in side_data:
            ...
            sigma_days = calibration.get(_lead_bucket(hours_to_peak), {}).get(
                "n_days", calibration.get(_lead_bucket(hours_to_peak), {}).get("n", 0)
            )
            sigma_source = "historical" if sigma_days >= 10 else "fallback"
            sigma_samples = sigma_days
            ...
            rows.append({
                ...
                "ask": ask,
                "bid": (
                    to_float(m.get("yes_bid_dollars"))
                    if side == "YES" else to_float(m.get("no_bid_dollars"))
                ),
                ...
                "previous_day_prediction_latest_snapshot_at": (
                    previous_day_prediction.get("latest_snapshot_at") if previous_day_prediction else None
                ),
                ...
                "nws_sigma_f": (
                    ...
                    * max(0.70, min(1.35, float(trajectory_live["sigma_multiplier"])))
                ),
                ...
                "trajectory_sigma_multiplier": trajectory_live["sigma_multiplier"],
                "nws_sigma_source": "historical" if calibration.get(_lead_bucket(hours_to_peak), {}).get("n_days", calibration.get(_lead_bucket(hours_to_peak), {}).get("n", 0)) >= 10 else "fallback",
                "nws_sigma_samples": calibration.get(_lead_bucket(hours_to_peak), {}).get("n_days", calibration.get(_lead_bucket(hours_to_peak), {}).get("n", 0)),
...
def previous_day_forecast_summary(city, tz_name, target_date):
    ...
    latest_row = projected.iloc[-1]
    return {
        "average_f": float(values.mean()),
        "low_f": float(values.min()),
        "high_f": float(values.max()),
        "latest_f": float(latest_row["projected_high_f"]),
        "latest_snapshot_at": latest_row["snapshot_at"],
...
def forecast_range_summary_chart(row, show_bet_overlay=False):
    ...
    if nws_center is None and nws_raw is not None:
        nws_center = nws_raw + nws_bias + trajectory_adjustment
        if observed is not None:
            nws_center = max(nws_center, observed)

    anchors = [
        v for v in [
            nws_raw, nws_center, observed, implied,
            clean_number(row.get("previous_day_high_f")),
            clean_number(row.get("previous_day_prediction_latest_f")),
            clean_number(row.get("previous_3day_avg_high_f")),
        ]
        if v is not None
    ] + gfs_values
    if not anchors:
    ...
    previous_day_nws_prediction = clean_number(row.get("previous_day_prediction_latest_f"))
    ...
    if previous_day_nws_prediction is not None:
        marker_specs.append(("Previous day NWS prediction", previous_day_nws_prediction, ""))
    ...
    if marker_rows:
        ...
        def marker_color(label):
            ...
            if label.startswith("Kalshi implied temp"):
                return "#FFD400"
            if label.startswith("Previous day NWS prediction"):
                return "#36C2FF"
...
def render_probability_chart_legend(row):
    ...
    if present(prev_pred_latest):
        items.append((
            f"Previous day NWS prediction · {float(prev_pred_latest):.1f}°F",
            "#36C2FF",
            "dash",
        ))
    ...
    if present(prev_pred_avg):
        if present(prev_pred_low) and present(prev_pred_high) and abs(float(prev_pred_high) - float(prev_pred_low)) >= 0.05:
            items.append((
                f"Previous-day forecast snapshot range · {prev_pred_n} snapshots",
                "#36C2FF",
                "history_band",
            ))
        else:
...
.bet-rationale strong {color:#D7CCFF;}
...
with st.sidebar:
    st.caption("Live market scan is cached for 5 minutes. Refreshes run supported cities in parallel.")
...
def concise_bet_blurb(r):
    """Return a compact data-driven explanation of the case for and risk in a bet."""
    try:
        chance = float(r.get("conservative_prob", 0.0))
    ...
    if edge_pp >= 20:
        strengths.append(f"WeatherEdge sees a large {edge_pp:.0f}-point probability edge")
    elif edge_pp >= 10:
        strengths.append(f"WeatherEdge sees a solid {edge_pp:.0f}-point probability edge")
    elif edge_pp > 0:
        strengths.append(f"WeatherEdge sees a {edge_pp:.0f}-point probability edge")

    strengths.append(
        f"the model puts this {r.get('side', '')} outcome at {chance*100:.0f}% versus a {ask*100:.0f}¢ ask"
    )

    traj = r.get("trajectory_agreement_score")
    if traj is not None and not pd.isna(traj):
        traj = float(traj)
        if traj >= 0.72:
            strengths.append("live observations are tracking the forecast reasonably well")
        elif traj < 0.45:
            risks.append("live temperatures are not tracking the forecast closely")

    hours_peak = r.get("hours_to_expected_peak")
    if hours_peak is not None and not pd.isna(hours_peak):
        hp = float(hours_peak)
        if hp <= 0.5:
            strengths.append("the day is at or past its expected hottest point")
        elif hp <= 2:
            strengths.append("the expected hottest point is close")
        elif hp > 5:
            risks.append("several hours of heating remain before the expected peak")

    sigma = r.get("nws_sigma_f")
    if sigma is not None and not pd.isna(sigma):
        sigma = float(sigma)
        if sigma >= 2.5:
            risks.append(f"historical forecast uncertainty is fairly wide at about ±{sigma:.1f}°F")
        elif sigma >= 1.7:
            risks.append(f"forecast uncertainty is still meaningful at about ±{sigma:.1f}°F")

    gap = r.get("trajectory_current_gap_f")
    if gap is not None and not pd.isna(gap):
        gap = float(gap)
        if abs(gap) >= 2:
            direction = "below" if gap < 0 else "above"
            risks.append(f"observations are running {abs(gap):.1f}°F {direction} the stored NWS trajectory")

    obs = r.get("observed_high_f")
    center = r.get("adjusted_nws_center_f")
    if obs is not None and center is not None and not pd.isna(obs) and not pd.isna(center):
        raw_center = (
            float(r.get("nws_high_f") or 0.0)
            + float(r.get("nws_bias_f") or 0.0)
            + float(r.get("trajectory_adjustment_f") or 0.0)
        )
        if raw_center < float(obs):
            strengths.append("the observed high now provides a hard floor to the final-temperature distribution")

    main = "; ".join(strengths[:3]).rstrip(".")
    if risks:
        concern = "; ".join(risks[:2]).rstrip(".")
        return f"{main}. Main concern: {concern}."
    return f"{main}. No major model warning is showing right now, though forecast and market uncertainty still remain."
...
def render_contract_detail(r, show_weather=True):
    ...
    if peak_left is not None and not pd.isna(peak_left):
        ...
        st.caption(
            ...
            f"{int(r.get('nws_sigma_samples', 0) or 0)} independent historical forecast-error days in this lead bucket."
        )
...
if main_view == "Best Bets":
    ...
    if not available_cities:
        ...
        with bet_panel:
            ...
            bid_text = (
                f" · Bid {float(bid)*100:.0f}¢"
                if bid is not None and not pd.isna(bid) else ""
            )
            spread_text = ""
            if bid is not None and not pd.isna(bid):
                spread_cents = max(0.0, (float(selected_row["ask"]) - float(bid)) * 100.0)
                spread_text = f" · Spread {spread_cents:.0f}¢"

            st.markdown(
                f"<div class='bet-callout-main'>{selected_row['side']} on "
                f"{selected_row['market_subtitle']}</div>"
                f"<div class='bet-callout-sub'>Ask {selected_row['ask']*100:.0f}¢{bid_text}{spread_text} · "
            ...
            bet_blurb = concise_bet_blurb(selected_row)
            if bet_blurb:
                st.markdown(
                    f"<div class='bet-rationale'><strong>Why this bet?</strong> {bet_blurb}</div>",
                    unsafe_allow_html=True,
                )
...
**NWS-based chance** converts the latest NWS high into a probability distribution using historical NWS-versus-observed errors across independent completed days. Multiple snapshots from the same day are collapsed before uncertainty is estimated, so dozens of correlated forecasts cannot make the distribution look artificially precise. Calibration is keyed to how far the forecast is from the city’s expected hottest point. Same-day live observations can adjust the center and moderately narrow uncertainty near the peak, but the model keeps a conservative uncertainty floor. GFS is not used in this probability.
