...
def previous_day_forecast_summary(city, tz_name, target_date):
    ...
    Returns dict with average_f, low_f, high_f, latest_f, n_snapshots, or None if unavailable.
    latest_f is the most recent stored NWS predicted high issued on source_day
    for the previous day's target, and is used as the chart's dotted reference line.
    """
    rows, err = get_snapshot_rows(city, None)
    snap, norm_err = normalize_snapshot_rows(rows)
    if err or norm_err or snap.empty:
